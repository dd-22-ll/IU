...
//------------------------------------------------------------------------------
uint32_t ulMedia_Get1MINHeadSize(uint8_t ubMaxVFPS, uint8_t ubMaxAFPS)
{
	uint32_t ulSize;
	switch(ubREC_FileFormatGet())
	{
		case REC_FILE_AVI:
			ulSize = AVI_INDEX_1MINSZ(ubMaxVFPS,ubMaxAFPS);
		break;
		case REC_FILE_MP4:
			ulSize = MP4_STBL_1MINSZ( ubMaxVFPS,ubMaxAFPS);
		break;
	}
	return ulSize;
}
...
//------------------------------------------------------------------------------
int32_t slMedia_WriteOneFrame(uint8_t ubCh, uint8_t ubStreamType, uint16_t uwFrameType, uint32_t ulAddr, uint32_t ulSize, uint32_t ulDesID, uint8_t ubFrmSeq)
{
	int32_t slRtn;
	switch(ubREC_FileFormatGet())
	{
		case REC_FILE_AVI:
			slRtn = slAVI_WriteOneFrame(ubCh, ubStreamType, uwFrameType, ulAddr, ulSize, NULL,ubFrmSeq);
		break;
		case REC_FILE_MP4:
			slRtn = slMP4_WriteOneFrame(ubCh, ubStreamType, uwFrameType, ulAddr, ulSize, ulDesID,ubFrmSeq);
		break;
	}
	return slRtn;
}
...
//------------------------------------------------------------------------------
void Media_Close(uint8_t ubCh, FS_FILE_SUB_HIDN_INFO_t HidnInfo)
{
	printf("MDA->Clo[%d]\n", ubCh);
	...
	{
		case REC_FILE_AVI:
			AVI_Close(ubCh, HidnInfo);
		break;
		case REC_FILE_MP4:
			MP4_Close(ubCh, HidnInfo);
		break;
	}
	return;
}

uint32_t ulMedia_AlignMoveLength(uint8_t ubCh, uint8_t ubStreamType)
{
    uint32_t ulRtn;
	switch(ubREC_FileFormatGet())
	{
		...
		break;
		case REC_FILE_MP4:
			ulRtn = ulMP4_AlignMoveLength(ubCh,ubStreamType);
		break;
	}
	return ulRtn;
}
void Media_ClearUserDataSize(uint8_t ubCh)
{
	switch(ubREC_FileFormatGet())
	{
		case REC_FILE_MP4:
			MP4_ClearUserDataSize(ubCh);
		break;
	}
	return;    
}

void Media_WriteUserData(uint8_t ubCh, uint8_t *ptr, uint32_t ulSize)
{
	switch(ubREC_FileFormatGet())
	{
		case REC_FILE_MP4:
			MP4_WriteUserData(ubCh,ptr,ulSize);
		break;
	}
	return;        
}
...
//------------------------------------------------------------------------------
void vMedia_GetHeadInfo(PLY_MP4HD_INFO *PlyMP4HdInfo)
{
    switch(ubREC_FileFormatGet())
    {
    	case REC_FILE_MP4:
    		vMP4_GetHeadInfo(PlyMP4HdInfo);
    	break;
    }
	return;
}
...
//------------------------------------------------------------------------------
void Media_ParsingAudioFormat(uint8_t ubCh, uint32_t ulAdr)
{
	switch(ubREC_FileFormatGet())
	{
		case REC_FILE_MP4:
			MP4_ParsingAudioFormat(ubCh, ulAdr);
		break;
	}
	return;
}
//------------------------------------------------------------------------------
uint8_t Media_AssignmentStructure(uint8_t ubCh, uint32_t ulAdr, uint32_t ulSize)
{
	switch(ubREC_FileFormatGet())
	{
		case REC_FILE_MP4:
			return MP4_AssignmentStructure(ubCh, ulAdr, ulSize);
		//break;
	}
	return 0;
}
...
uint8_t ubMedia_GetAdoCodec(uint8_t ubCh)
{
	uint8_t ubCodec;
	switch(ubREC_FileFormatGet())
	{
		...
		break;
		case REC_FILE_MP4:
			ubCodec = ubMP4_GetAudioCodec(ubCh);
		break;
	}
	return ubCodec;
}
...
//------------------------------------------------------------------------------
uint32_t ulMedia_Parse1stIFrameInfo(uint8_t ubCh, uint32_t ulAdr,PLY_CHUNK_CONTROLLER *PlyChkCtrl)
{
	uint32_t ulRtn;
	switch(ubREC_FileFormatGet())
	{
		...
		break;
		case REC_FILE_MP4:
			ulRtn = ulMP4_Parse1stIFrameInfo(ubCh, ulAdr, PlyChkCtrl);
		break;
	}
	return ulRtn;
}
