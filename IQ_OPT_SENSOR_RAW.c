/*!
	...
	\file		IQ_OPT_SENSOR_RAW.c
	\brief		Dynamic image quality relation function
	\author		
	\version	1.2
	\date		2020/11/09
...
void IQ_DynamicCCM_4G5(void) {
	...
	ulAeIdxGainTmp = ((uint32_t)xtIQDnInst.xtIQJudgeInst.uwAE_CurrGain * (uint32_t)xtIQDnInst.xtIQJudgeInst.ubAE_Expidx);
...
void IQ_DynamicDenoise_4G5(void) {
	...
	if (ubSEN_GetDayNightMode() == DAY_MODE)
    {
        //Day mode
        ...
        }else if(slIQ_DayNrRes2Linear[1][0] == xtIQDnInst.uwCurrPrevHSz){
        ...
        }else if(slIQ_DayNrRes3Linear[1][0] == xtIQDnInst.uwCurrPrevHSz)
        {
    ...
    }else if (ubSEN_GetDayNightMode() == NIGHT_MODE){
        //Night mode
        ...
        }else if(slIQ_NightNrRes2Linear[1][0] == xtIQDnInst.uwCurrPrevHSz){
        ...
        }else if(slIQ_NightNrRes3Linear[1][0] == xtIQDnInst.uwCurrPrevHSz){
...
//------------------------------------------------------------------------------
void IQ_DynamicAbsY(void) {
	uint8_t i;
	int32_t slAbsY_Temp[0x40];
	
	IQ_DynamicLinearInterpolation(xtIQDnInst.xtIQJudgeInst.ulAE_AbsY, slIQ_AbsyLinear[0][1], sizeof(slIQ_AbsyLinear[0]) / sizeof(int32_t), &slIQ_AbsyLinear[3][0], slAbsY_Temp);
	for (i = 0; i < slIQ_LscLinear[0][0]; i ++) {
		IQ_SetBinVal(slIQ_AbsyLinear[0][0 + i + 2], slIQ_AbsyLinear[1][0 + i + 2], slIQ_AbsyLinear[2][0 + i + 2], slAbsY_Temp[i]); 
	}
}
