...
#define I2C_MINORVER    6        //!< Minor version
//------------------------------------------------------------------------------
...
I2C1_Type *pI2C_MasterInit (I2C_TYP tNth, I2C_SCL_SPEED_TYP tSclSpeed)
{
	...
	I2C_Reset(pI2C);
	pI2C->I2C_SCL_COUNT = ((480000 >> GLB->SYS_RATE) / GLB->APBC_RATE / tSclSpeed - pI2C->I2C_GSR) / 2 - 2;
	pI2C->I2C_EN = 1;	
...
bool bI2C_MasterIntRwInternal (I2C1_Type *pI2C, uint8_t ubMode, uint8_t ubAddr, uint8_t *pBuf, uint32_t ulSz)
{
	...
	if (true == bI2C_MasterReady(pI2C))
	{
		...
		if (I2C1 == pI2C)
		{
			...
			INTC_IrqSetup (INTC_I2C1_IRQ, I2C_Master1Isr);
			INTC_IrqClear (INTC_I2C1_IRQ);
		...
		else
		{
			...
			INTC_IrqSetup (INTC_I2C2_IRQ, I2C_Master2Isr);
			INTC_IrqClear (INTC_I2C2_IRQ);
...
//	return false;
	...
	pMutex = (I2C1 == pI2C)?&I2C_Mutex1:&I2C_Mutex2;
	osMutexWait(*pMutex, osWaitForever);
    memcpy((uint8_t *)&ubWrBuf[0], pWrBuf, ulWrSz);
    memcpy((uint8_t *)&ubRdBuf[0], pRdBuf, ulRdSz);
    
    if(ulWrSz)
    {
        ubRet = bI2C_MasterIntRw (pI2C, ubMode, ubWrAddr, ubWrBuf, ulWrSz);
    }
    if(ulRdSz)
    {
       ubRet = bI2C_MasterIntRw (pI2C, ubMode, ubRdAddr, ubRdBuf, ulRdSz);  
        memcpy(pRdBuf, (uint8_t *)&ubRdBuf[0], ulRdSz);
    }
    osMutexRelease(*pMutex);
	return ubRet;
}
...
bool bI2C_SlaveIntInternal (I2C1_Type *pI2C, uint8_t ubMode, I2C_SLAVE_RW_TYP tRw, uint8_t *pBuf, uint32_t ulSz)
{
	...
	if (I2C1 == pI2C)
	{
		pubI2C_BufIsr1 = pBuf;	
		ulI2C_CountIsr1 = 0;		
		INTC_IrqSetup (INTC_I2C1_IRQ, I2C_Slave1Isr);
		INTC_IrqClear (INTC_I2C1_IRQ);
		INTC_IrqEnable (INTC_I2C1_IRQ);
		ubI2C_IsrFlag1 = 0;
	...
	else
	{
		pubI2C_BufIsr2 = pBuf;	
		ulI2C_CountIsr2 = 0;
		INTC_IrqSetup (INTC_I2C2_IRQ, I2C_Slave2Isr);
		INTC_IrqClear (INTC_I2C2_IRQ);
		INTC_IrqEnable (INTC_I2C2_IRQ);
		ubI2C_IsrFlag2 = 0;
