...
#define OSD_MINORVER    4        //!< Minor version
//------------------------------------------------------------------------------
...
static uint16_t					 uwOSD_HDmSize = 0;
//------------------------------------------------------------------------------
...
{
	return uwOSD_HSize - uwOSD_GetHDummyPixel();
}
//------------------------------------------------------------------------------
...
void OSD_UpdatePattern (OSD_BUF_TYP *pOsdBuf)
{
	...
	//OSD_Disable();		//huang 20241211
	OSD_Enable();		//huang 20241211
...
{
	uint8_t ubDmSize = 0;
	ubDmSize = ((uwHSize > 1000) && (uwHSize < 1052))?64:0;
	ulOSD_BufSize = (uwHSize + ubDmSize) * uwVSize + (OSD_PAT_NUM << 1);
	if (3 & ulOSD_BufSize)
...
void OSD_SfUpdateData (uint32_t ulSfAddr, uint32_t ulBufAddr, uint32_t ulLen)
{
	...
	tInfo.tWaitRdyMethod  = SF_WAIT_IN_LOWER_PRIO;
	tInfo.ubCrcOrder 	  = 16;
...
void OSD_LogoJpeg (uint32_t ulLogo_Index)
{
	...
	if(!ulImgMaxSize)
	{
		printd(DBG_ErrorLvl, "Logo Max Size error !\n");
		osMutexRelease(OSD_Mutex);
		return;
	}
	ulImgMaxSize *= 1024;
	if(LCD_JPEG_DISABLE == tLCD_GetJpegDecoderStatus())
	{
		...
		printd(DBG_CriticalLvl, "Load Jpeg info.\n");
	}
...
//------------------------------------------------------------------------------
uint32_t OSD_LostLinkAppImg (uint32_t ubLostLinkAppImgBuf)
{
	uint32_t 	   ulLostLinkAppImgSFAddr;
	uint32_t 	   ulImgMaxSize;
	uint32_t	   ulLostLinkAppImgAddr;
	uint32_t	   ulLostLinkAppImgSize;

	osMutexWait(OSD_Mutex, osWaitForever);

	ulLostLinkAppImgSFAddr = *((uint32_t *)(pbPROF_GetParam(OSDPARAM) + OSD_FONT_ADDR));
	
	if (ulLostLinkAppImgSFAddr == 0xFFFFFFFF || ulLostLinkAppImgSFAddr == 0)
	{
		printd(DBG_ErrorLvl, "No Lost Link Img !\n");
		osMutexRelease(OSD_Mutex);
		return 0;
	}
	
	SF_Read(ulLostLinkAppImgSFAddr + OSD_LOGO_MAXSIZE_SFT, OSD_LOGO_JPEG_MAXSIZE_LEN, (uint8_t*)&ulImgMaxSize);

	if(!ulImgMaxSize)
	{
		printd(DBG_ErrorLvl, "Lost Link Max Size error !\n");
		osMutexRelease(OSD_Mutex);
		return 0;
	}

	ulLostLinkAppImgAddr = ulLostLinkAppImgSFAddr + (0 * ulImgMaxSize) + OSD_LOGO_HEADER_LEN;
	SF_Read(ulLostLinkAppImgSFAddr + ((0 * 4) + OSD_LOGO_BSLEN_SFT), OSD_LOGO_JPEG_FILESIZE_LEN, (uint8_t*)&ulLostLinkAppImgSize);
	printd(DBG_Debug3Lvl, "Image Size 0x%X[(0x%X)(0x%X)]\n", ulLostLinkAppImgSFAddr, ulLostLinkAppImgAddr, ulLostLinkAppImgSize);

	SF_Read(ulLostLinkAppImgAddr, ulLostLinkAppImgSize, (uint8_t*)ubLostLinkAppImgBuf);

	printd(DBG_Debug3Lvl, "Img buf: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n", 
		*(uint8_t*)(ubLostLinkAppImgBuf), *(uint8_t*)(ubLostLinkAppImgBuf+1), *(uint8_t*)(ubLostLinkAppImgBuf+2), *(uint8_t*)(ubLostLinkAppImgBuf+3),
		*(uint8_t*)(ubLostLinkAppImgBuf+4), *(uint8_t*)(ubLostLinkAppImgBuf+5), *(uint8_t*)(ubLostLinkAppImgBuf+6), *(uint8_t*)(ubLostLinkAppImgBuf+7));

	osMutexRelease(OSD_Mutex);

	return ulLostLinkAppImgSize;
}
//------------------------------------------------------------------------------
OSD_RESULT tOSD_Init (OSD_WEIGHT_TYP tOsdWeight, uint32_t ulHSize, uint32_t ulVSize, uint16_t uwXStart, uint16_t uwYStart, OSD_SCALE_TYP tXScale, OSD_SCALE_TYP tYScale)
{
	...
	if (!(ulHSize & 3))
	{
		...
		if (uwLCD_GetLcdHoSize() >= ulXEnd && uwLCD_GetLcdVoSize() >= ulYEnd)
		{
			...
			uwOSD_HSize = ulHSize;
			uwOSD_VSize = ulVSize;
			OSD_SetHDummyPixel(28);
			ulHSize += uwOSD_GetHDummyPixel();
			uwOSD_HSize = ulHSize;
...
//------------------------------------------------------------------------------
#define OSD_IMG_HEADER_SFT 	(0x10)
#define OSD_IMG_HSIZE_SFT 	(OSD_IMG_HEADER_SFT + 8)
#define OSD_IMG_VSIZE_SFT 	(OSD_IMG_HEADER_SFT + 0xA)
uint16_t uwOSD_OsdImgHsize = 0;	
uint16_t uwOSD_OsdImgVsize = 0;	
//------------------------------------------------------------------------------
uint16_t uwOSD_GetOsdImgHsize (void)
{	
	if (!uwOSD_OsdImgHsize)
	{
		tOSD_SfAddr.ulOsdImgSfStartAddr = 
		*((uint32_t *)(pbPROF_GetParam(OSDPARAM) + OSD_IMAGE_ADDR));
		SF_Read(tOSD_SfAddr.ulOsdImgSfStartAddr + OSD_IMG_HSIZE_SFT, 
				 sizeof(uwOSD_OsdImgHsize), (uint8_t*)&uwOSD_OsdImgHsize);
	}
	return uwOSD_OsdImgHsize;
}
//------------------------------------------------------------------------------
uint16_t uwOSD_GetOsdImgVsize (void)
{
	if (!uwOSD_OsdImgVsize)
	{
		tOSD_SfAddr.ulOsdImgSfStartAddr = 
		*((uint32_t *)(pbPROF_GetParam(OSDPARAM) + OSD_IMAGE_ADDR));
		SF_Read(tOSD_SfAddr.ulOsdImgSfStartAddr + OSD_IMG_VSIZE_SFT, 
				 sizeof(uwOSD_OsdImgVsize), (uint8_t*)&uwOSD_OsdImgVsize);
	}
	return uwOSD_OsdImgVsize;
}
//------------------------------------------------------------------------------
void OSD_ResetOsdStart (uint8_t ubX, uint8_t ubY)
{	
	LCD->OSD_X_END = uwLCD_GetLcdHoSize() - ubX;
	LCD->OSD_X_START = LCD->OSD_X_END - uwOSD_OsdImgHsize;
	LCD->OSD_Y_END = LCD->OSD_Y_END + ubY - LCD->OSD_Y_START;
	LCD->OSD_Y_START = ubY;
}
//------------------------------------------------------------------------------
void OSD_SetHDummyPixel(uint8_t ubDM_Size)
{
	uint16_t uwLcd_H = 0, uwOSD_H = 0;
	uwLcd_H = uwLCD_GetLcdHoSize();
	uwOSD_H = uwOSD_GetHSize();
	if((uwLcd_H >= uwOSD_H) && ((uwLcd_H > 1000) && (uwLcd_H < 1052)))
	{
		uwOSD_HDmSize = (ubDM_Size & 3)?(ubDM_Size + (4 - (ubDM_Size % 4))):ubDM_Size;
		LCD->LCD_HO_SIZE = uwLcd_H + uwOSD_HDmSize;
	}
}
//------------------------------------------------------------------------------
uint16_t uwOSD_GetHDummyPixel(void)
{
	return uwOSD_HDmSize;
}
