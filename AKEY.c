...
void AKEY_Init(void)
{
	...
	tAKEY_AinCh2 = SADC_CH2;	//huang	//SADC_CH1
	SADC_Init();
	SADC_SetPinMode(tAKEY_AinCh2, SADC_PIN_ANALOG_INPUT_MODE);
	SADC_Enable();
}
//------------------------------------------------------------------------------
...
AKEY_ID_t AKEY_Detection(void)
{
	...
	//SADC
	uwAKEY_DetLvl = uwSADC_GetReport(tAKEY_AinCh2);
	
	akey_count++;
	if (akey_count >= 100)
	{	akey_count = 0;
		printf("uwAKEY_DetLvl = %d\n", uwAKEY_DetLvl);		//huang
	}
