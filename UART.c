...
#define UART_MINORVER	6
//------------------------------------------------------------------------------
const uint32_t ulUART_DevAddr[] = {UART1_BASE, UART2_BASE};
const INTC_IrqSrc_t iUART_IRQn[] = {INTC_UART1_IRQ, INTC_UART2_IRQ};
const INTC_IrqHandler iUART_IrqHandler[] = {UART1_INT_Handler, UART2_INT_Handler};
uint8_t ubUART_Ready[] = {UART_NOTREADY, UART_NOTREADY};										//!< Ready after initialize
static pvUARTRecvCbFunc pvUART_Recv[2];
#ifdef RTOS
osSemaphoreId *pxUART_RxSemaphore[] = {NULL, NULL};
#endif
//------------------------------------------------------------------------------
void UART_RegistRecvCb(UART_DEVICE_t tUART_Dev, pvUARTRecvCbFunc pv)
{
	pvUART_Recv[tUART_Dev] = pv;
}
//------------------------------------------------------------------------------
void UART_Init(UART_DEVICE_t tUART_Dev, UART_CLOCK_RATE_t tClockRate, UART_BAUDRATE_t tBaudRate, uint8_t ubStopbit, void* pxRxSemaphore)
{
	...
	if(pxUART_RxSemaphore[tUART_Dev] == NULL)
	{
		...
		INTC_IrqSetup(iUART_IRQn[tUART_Dev], iUART_IrqHandler[tUART_Dev]);
...
void UART1_INT_Handler(void)
{
	...
	while(UART1->RX_RDY)
	{
		...
		if (pvUART_Recv[UART_1])
			pvUART_Recv[UART_1](ch);
	}
#ifdef RTOS
...
void UART2_INT_Handler(void)
{
	...
	while(UART2->RX_RDY)
	{
		...
		if (pvUART_Recv[UART_2])
			pvUART_Recv[UART_2](ch);
	}
#ifdef RTOS
		/*! We have not woken a task at the start of the ISR. */
