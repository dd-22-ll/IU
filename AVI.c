...
FS_MEDIA_SEL AVI_FS_MediaSel = FS_MEDIA_0;
...
void AVI_UpdateHeader(uint8_t ubCh)
{
	...
	{
		FS_WriteFile(AVI_FS_MediaSel, (FS_SRC_NUM)ubCh, (uint32_t)(AVI_Idx1[ubCh]), AVI_Idx1[ubCh]->cb + AVI_BYTE_OF_HEADER,NULL);//update index
...
//	ulSize: Data size.
//------------------------------------------------------------------------------
int32_t slAVI_WriteOneFrame(uint8_t ubCh, uint8_t ubStreamType, uint16_t uwFrameType, uint32_t ulAddr, uint32_t ulSize, uint32_t ulDesID, uint8_t ubFrmSeq)
{
	...
	{
		FS_WriteFile(AVI_FS_MediaSel, (FS_SRC_NUM)ubCh, ulSkipFrameBuf, ulAVI_SfTotalSize[ubCh], NULL);
	...
	if ((uwFrameType != AVI_V_SFRM))
	{
		...
		FS_WriteFile(AVI_FS_MediaSel, (FS_SRC_NUM)ubCh, (uint32_t)(ubFrameTag), 8, NULL);
	...
	if (uwFrameType != AVI_V_SFRM)
	{
		...
		FS_WriteFile(AVI_FS_MediaSel, (FS_SRC_NUM)ubCh, (uint32_t)ulAddr, ulSize, NULL);
...
void AVI_CreateFile(uint8_t ubCh)
{
	...
	FS_WriteFile(AVI_FS_MediaSel, (FS_SRC_NUM)ubCh, (uint32_t)(&AVIS_Rec[ubCh].RIFF), sizeof(AVIS_Rec[0].RIFF) + sizeof(AVIS_Rec[0].hl) + sizeof(AVIS_Rec[0].info), NULL);	// 500 Bytes
	FS_WriteFile(AVI_FS_MediaSel, (FS_SRC_NUM)ubCh, ulThumbnailAdr, AVI_THUMBNAIL_SIZE, NULL);		// 32.5K Bytes
	FS_WriteFile(AVI_FS_MediaSel, (FS_SRC_NUM)ubCh, (uint32_t)(&AVI_RecMovi[ubCh]), sizeof(AVICHUNK), NULL);	//12 Bytes
}

//------------------------------------------------------------------------------
//	AVI Close
//------------------------------------------------------------------------------
void AVI_Close(uint8_t ubCh, FS_FILE_SUB_HIDN_INFO_t HidnInfo)
{
	if (AVIS_Rec[ubCh].hl.avih.ulTotalFrames == 0)			//if have no any video frame, add a skip frame for Media Player can play normally.
	{
		...
		slAVI_WriteOneFrame(ubCh, AVI_STR_V1, AVI_V_SFRM, 0, ulAVI_GetSkipFrameSize(), NULL,NULL);
	}
	...
	FS_UpdateAviHeader1(AVI_FS_MediaSel, (FS_SRC_NUM)ubCh,(uint32_t)&AVIS_Rec[ubCh].RIFF, sizeof(AVIS_Rec[ubCh].RIFF) + sizeof(AVIS_Rec[ubCh].hl) + sizeof(AVIS_Rec[ubCh].info));
	FS_UpdateAviHeader2(AVI_FS_MediaSel, (FS_SRC_NUM)ubCh,(uint32_t)&AVI_RecMovi[ubCh], sizeof(AVICHUNK));
	
	FS_CloseFile(AVI_FS_MediaSel, (FS_SRC_NUM)ubCh, &HidnInfo);
