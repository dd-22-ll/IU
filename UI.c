...
extern OSD_WEIGHT_TYP transparency_levels[];
#endif
#ifdef VBM_BU
#include "UI_VBMBU.h"
#endif
...
#define MENU_PAIR_MODE_GUIDE         30
#define MENU_PAIR_MODE_ONE           31
#define MENU_PAIR_MODE_TWO           32
#define MENU_PAIR_MODE_THREE         33
#define MENU_SLEEP_HISTORY           34
#define MENU_SLEEP_HISTORY_VIEW      35
#define MENU_SLEEP_HISTORY_OPTIONS   36
#define MENU_SLEEP_HISTORY_SELECT    37
#define MENU_SLEEP_HISTORY_DELETED   38
#define MENU_CHANGE_PRIORITY_MAIN    39
#define MENU_CHANGE_PRIORITY_PICK    40
#define MENU_PAIR_RUN                41
#define MENU_PAIR_SUCCESS            42
#define MENU_UNPAIR_UNITS            43
#define MENU_UNPAIR_BABY_UNIT        44
#define MENU_BABY_UNIT_WAS_UNPAIRED  45
#define MENU_PAIR_PROGRESS  				 46
...
const Point TEMP_HIGH_MENU_POS[] = {
    {40, 140}, {180, 140}, {320, 140}, {460, 140}, {40, 180}, {180, 180}, {320, 180}, {460, 180}, {40, 220}, {180, 220}, {320, 220}, {460, 220}, {50, 260}};
...
const Point TEMP_LOW_MENU_POS[] = {
    {40, 140}, {180, 140}, {320, 140}, {460, 140}, {40, 180}, {180, 180}, {320, 180}, {460, 180}, {40, 220}, {180, 220}, {320, 220}, {460, 220}, {40, 260}, {180, 260}, {50, 300}};
...
// PairMode
static uint16_t PAIRMODE_MENU_Y_POS[]   = {150, 190, 230, 270};
static uint8_t PAIRMODE_MENU_ITEM_COUNT = sizeof(PAIRMODE_MENU_Y_POS) / sizeof(uint16_t);
// PairMode_one
static uint8_t g_pair_target_cnt         = 1;
static uint16_t PAIRMODE_GUIDE_Y_POS[]   = {360, 400};
static uint8_t PAIRMODE_GUIDE_ITEM_COUNT = 2;
// Pair wizard
static uint8_t g_pair_needed = 0; // target amount(= g_pair_target_cnt)
static uint8_t g_pair_done   = 0; // already amount
// 0=STAND-BY, 1=SUCCESS
static uint8_t g_pair_status[3] = {0, 0, 0};
// Unpair
static uint8_t UNPAIR_MENU_Y_POS[6];
static uint8_t UNPAIR_MENU_ITEM_COUNT = 0;
static uint8_t g_unpair_pick_menu_idx = 0;
static uint8_t g_unpair_target_ch     = 0xFF;
// ChangePriority
static uint16_t PRIORITY_MENU_Y_POS[]   = {107, 147};
static uint8_t PRIORITY_MENU_ITEM_COUNT = sizeof(PRIORITY_MENU_Y_POS) / sizeof(uint16_t);

// --- Change-Priority runtime state ---
static uint8_t g_pri_step     = 0;
static uint8_t g_pri_order[3] = {0xFF, 0xFF, 0xFF};
static uint8_t g_pri_mask     = 0;
static bool g_pri_msg_show    = false;
...
void UI_ToggleDigitalZoom(void)
{
    ...
    } else // Current is 2x, switching to 1x
    {
        ...
        dyn_info.tChRes.uwCropHstart = 0;
        dyn_info.tChRes.uwCropVstart = 0;
    }

    result = tLCD_DynamicOneChCropScale(&dyn_info);
...
void UI_ShowSetting(void)
{
    ...
    if (tUI_PuSetting.ubVibration == 0) {
        UI_DrawReverseString23(vibration_str, 527, 200);
    } else if (tUI_PuSetting.ubVibration == 1) {
        UI_DrawReverseString23(vibration_str, 526, 200);
    } else if (tUI_PuSetting.ubVibration == 2) {
...
{
    MenuBackground();
    UI_DrawString("PAIR MODE", 225, 52);
    ...
    UI_DrawString("1", 52, 156);
    UI_DrawString("2", 52, 196);
    UI_DrawString("3", 52, 236);
    UI_DrawString("CANCEL", 52, 276);
...
void UI_PairModeGuide(void)
{
    ...
    if (g_pair_target_cnt == 1) {
        UI_DrawString23("1    TURN ON THE BABY UNIT YOU WANT", 52, 104);
        UI_DrawString23("      TO PAIR.", 53, 129);

        UI_DrawString23("2    PRESS AND HOLD THE PAIR BUTTON ON THE", 52, 164);
        UI_DrawString23("      BABY UNIT FOR 5 SECONDS.", 55, 189);
        UI_DrawString23("      (BOTTOM OF THE BABY UNIT).", 55, 214);
        UI_DrawString23("      NIGHT LIGHT FLASHES WHEN BABY UNIT IS", 55, 239);
        UI_DrawString23("      IN PAIR MODE.", 55, 264);

        UI_DrawString23("3    WHEN THE NIGHT LIGHT ON THE BABY UNIT", 52, 299);
        UI_DrawString23("      IS FLASHING, CLICK CONTINUE.", 55, 324);
    } else if (g_pair_target_cnt == 2) {
        UI_DrawString23("1    TURN ON THE TWO BABY UNITS YOU WANT", 52, 104);
        UI_DrawString23("      TO PAIR.", 53, 129);

        UI_DrawString23("2    PRESS AND HOLD THE PAIR BUTTON ON BOTH", 52, 164);
        UI_DrawString23("      BABY UNITS FOR 5 SECONDS.", 55, 189);
        UI_DrawString23("      (BOTTOM OF THE BABY UNIT).", 55, 214);
        UI_DrawString23("      NIGHT LIGHT FLASHES WHEN BABY UNIT IS", 55, 239);
        UI_DrawString23("      IN PAIR MODE.", 55, 264);

        UI_DrawString23("3    WHEN THE NIGHT LIGHT ON BOTH BABY UNITS", 52, 299);
        UI_DrawString23("      ARE FLASHING, CLICK CONTINUE.", 55, 324);
    } else { // g_pair_target_cnt == 3
        UI_DrawString23("1    TURN ON ALL BABY UNITS YOU WANT", 52, 104);
        UI_DrawString23("      TO PAIR.", 53, 129);

        UI_DrawString23("2    PRESS AND HOLD THE PAIR BUTTON ON ALL", 52, 164);
        UI_DrawString23("      BABY UNITS FOR 5 SECONDS.", 55, 189);
        UI_DrawString23("      (BOTTOM OF THE BABY UNIT).", 55, 214);
        UI_DrawString23("      NIGHT LIGHT FLASHES WHEN BABY UNIT IS", 55, 239);
        UI_DrawString23("      IN PAIR MODE.", 55, 264);

        UI_DrawString23("3    WHEN THE NIGHT LIGHT ON ALL BABY UNITS", 52, 299);
        UI_DrawString23("      ARE FLASHING, CLICK CONTINUE.", 56, 324);
    }

    UI_DrawString("CONTINUE", 52, PAIRMODE_GUIDE_Y_POS[0] + 8);
    UI_DrawString("EXIT PAIR MODE", 52, PAIRMODE_GUIDE_Y_POS[1] + 8);

    if (g_current_menu_index >= PAIRMODE_GUIDE_ITEM_COUNT)
        g_current_menu_index = 0;
    g_rectangle_x = 40;
    g_rectangle_y = PAIRMODE_GUIDE_Y_POS[g_current_menu_index];
...
void UI_ShowPairRun(void)
{
    ...
    for (uint8_t i = 0; i < g_pair_needed; ++i) {
        ...
        if (g_pair_needed == 1) {
            ...
            UI_DrawString23(l, 40, y);
        }
        if (g_pair_status[i] == 0) {
            UI_DrawString23(" STAND-BY", 200, y);
        } else {
            UI_DrawString23(" PAIRING SUCCESSFUL", 200, y);
        }
    }
...
void UI_ShowPairSuccess(void)
{
    MenuBackground();
    UI_DrawString("GREAT SUCCESS", 190, 52);
...
void UI_Pairing_OnBuPaired(void)
{
    if (g_current_menu_level != MENU_PAIR_RUN) return;

    if (g_pair_done >= g_pair_needed) return;

    g_pair_status[g_pair_done] = 1;
    g_pair_done++;

    if (g_pair_done >= g_pair_needed) {
        g_current_menu_level = MENU_PAIR_SUCCESS;
        g_current_menu_index = 0;
        g_rectangle_x        = 40;
        g_rectangle_y        = 110;
...
void UI_UpdateUnpairMenuPositions(void)
{
    uint8_t bu_count = 0;
    for (uint8_t i = 0; i < 3; ++i) {
        if (ubTRX_GetLinkStatus(i)) {
    ...
    if (g_current_menu_index >= UNPAIR_MENU_ITEM_COUNT)
        g_current_menu_index = 0;

    g_rectangle_x = 40;
    g_rectangle_y = UNPAIR_MENU_Y_POS[g_current_menu_index];
}

void UI_ShowUnpairUnits(void)
{
    MenuBackground();
    UI_DrawString("UNPAIR UNITS", 210, 52);
    ...
    UI_DrawString("EXIT", 52, UNPAIR_MENU_Y_POS[UNPAIR_MENU_ITEM_COUNT - 1] + 8);

    UI_Drawbox();
}

void UI_ShowUnpairBabyUnit(void)
{
    MenuBackground();
    UI_DrawString("UNPAIR BABY UNIT", 180, 52);
    ...
    if (g_current_menu_index > 1) g_current_menu_index = 0;
    g_rectangle_x = 40;
    g_rectangle_y = (g_current_menu_index == 0) ? yes_y : no_y;

    UI_Drawbox();
}

void UI_ShowBabyUnitWasUnpaired(void)
{
    MenuBackground();
    UI_DrawString("BABY UNIT WAS UNPAIRED", 140, 52);
...
void UI_SleepHistoryView(void)
{
    ...
    for (int i = 0; i < CAM_4T; ++i) {
        if (tUI_CamStatus[i].sleep_hist_count > 0) {
            unit = i;
            break;
        }
    }
...
void DisplaySettings(void)
{
    ...
    if (tUI_PuSetting.ubZoomLevel == 0) {
        UI_DrawReverseString23("OFF", 327, 160);
    } else {
        UI_DrawReverseString23("2X", 335, 160);
    }
    switch (tUI_PuSetting.ubFlipImage) {
        case 0:
            UI_DrawReverseString23("0"
                                   "\xB0",
                                   340, 200);
            break;
        case 1:
            UI_DrawReverseString23("90"
                                   "\xB0",
                                   334, 200);
            break;
        case 2:
            UI_DrawReverseString23("180"
                                   "\xB0",
                                   327, 200);
            break;
        case 3:
            UI_DrawReverseString23("270"
                                   "\xB0",
                                   325, 200);
            break;
    }
...
void AdvancedDisplaySettings(void)
{
    ...
    UI_DrawHalfWhiteSquare(508, 110);
    const char *transparency_text[] = {"0%", "20%", "40%", "60%"};
    if (tUI_PuSetting.ubTextTransparency == 0) {
        UI_DrawReverseString23(transparency_text[tUI_PuSetting.ubTextTransparency], 540, 120);
    } else {
        UI_DrawReverseString23(transparency_text[tUI_PuSetting.ubTextTransparency], 530, 120);
    }
    UI_Drawbox();
}
...
void UI_ShowBabyUnitDetail(uint8_t unit_index)
{
    ...
    tOSD_Img2(&tOsdImgInfoLine1, OSD_QUEUE);
		...
		if (tUI_CamStatus[unit_index].active)
				UI_DrawReverseString23("ACTIVE", 455, y + 10);
		else
				UI_DrawReverseString23("INACTIVE", 455, y + 10);

		y += 40;

		if (ubTRX_GetLinkStatus(unit_index)) {
				UI_DrawString("CHANGE NAME", 52, y + 8);
				y += 40;
				UI_DrawString("INFO", 52, y + 8);
				y += 40;
    ...
    if (g_current_menu_index == 0) {
        if (tUI_CamStatus[unit_index].active==1) {
            OSD_IMG_INFO tOsdImgInfoLine2;
            tOSD_GetOsdImgInfor(1, OSD_IMG2, OSD2IMG_OSD2HORIZONTALLINE, 1, &tOsdImgInfoLine2);
            tOsdImgInfoLine2.uwXStart = 40;
...
void UI_ShowChangeName(void)
{
    ...
    UI_DrawString23(ENGLISH_KEYBOARD[2][4], x, y); // SPACE
    x += 100;
    UI_DrawString23(ENGLISH_KEYBOARD[2][5], x, y); // DELETE
...
//------------------------------------------------------------------------------
void UI_UpdateChangePriorityPositions(void)
{
    uint8_t connected_count = 0;
    for (uint8_t i = 0; i < 3; i++) {
        if (ubTRX_GetLinkStatus(i)) {
    ...
    if (connected_count == 0) {
        base_y                 = 110;
        menu_item_count        = 1;
        PRIORITY_MENU_Y_POS[0] = base_y; // EXIT
    } else if (connected_count == 1) {
        base_y                 = 107 + 20;
        menu_item_count        = 1;
        PRIORITY_MENU_Y_POS[0] = base_y;
    } else if (connected_count == 2) {
        base_y                 = 107 + 40;
        menu_item_count        = 2;
        PRIORITY_MENU_Y_POS[0] = base_y;
        PRIORITY_MENU_Y_POS[1] = base_y + 40; // Changepriority
    } else {                                  // connected_count == 3
        base_y                 = 107 + 60;
        menu_item_count        = 2;
        PRIORITY_MENU_Y_POS[0] = base_y;
        PRIORITY_MENU_Y_POS[1] = base_y + 40;
    }

    PRIORITY_MENU_ITEM_COUNT = menu_item_count;
}

void UI_ShowChangePriorityMain(void)
{
    MenuBackground();
    UI_DrawString("CHANGE PRIORITY", 180, 52);
    ...
    if (connected_count == 0) {
        UI_DrawString("EXIT", 52, current_y + 17);
    } else if (connected_count == 1) {
        ...
        for (uint8_t i = 0; i < 3; i++) {
            if (ubTRX_GetLinkStatus(i)) {
                char line[12];
                UI_DrawString23(tUI_CamStatus[i].name, 52, current_y + 5);
                snprintf(line, sizeof(line), "PRIORITY %u", (unsigned)(tUI_CamStatus[i].priority + 1));
                UI_DrawString23(line, 390, current_y + 5);
                current_y += 20;
                break;
            }
        }
        ...
        UI_DrawString("CHANGE PRIORITY", 52, current_y + 5);
        current_y += 40;
        UI_DrawString("EXIT", 52, current_y + 5);
    ...
    UI_UpdateChangePriorityPositions();

    if (g_pri_msg_show) {
        UI_DrawString23("NEW PRIORITY HAS BEEN SET.", 40, 430);
    }

    UI_Drawbox();
}

static void UI_ShowChangePriorityPick(void)
{
    MenuBackground();
    UI_DrawString("CHANGE PRIORITY", 200, 52);
    ...
    char ttl[32];
    snprintf(ttl, sizeof(ttl), "SET PRIORITY %u", (unsigned)g_pri_step);
    UI_DrawString(ttl, 40, 110);

    uint8_t avail[3];
    uint8_t n = 0;
    for (uint8_t k = 0; k < 3; ++k) {
        uint8_t ch = g_bu_idx_map[k];
        if (ubTRX_GetLinkStatus(ch) && !(g_pri_mask & (1u << ch))) {
            avail[n++] = ch;
        }
    }

    uint16_t y = 147;
    for (uint8_t i = 0; i < n; ++i, y += 40) {
        UI_DrawString23(tUI_CamStatus[avail[i]].name, 52, y);
        PRIORITY_MENU_Y_POS[i] = y - 12; // ??????
    }

    // EXIT
    UI_DrawString("EXIT", 52, y + 8);
    PRIORITY_MENU_Y_POS[n] = y - 12;

    uint8_t max = n; // + EXIT
    if (g_current_menu_index > max) g_current_menu_index = max;
    g_rectangle_x = 40;
    g_rectangle_y = PRIORITY_MENU_Y_POS[g_current_menu_index];
...
void UI_ShowSetLimitHigh(void)
{
    ...
    sprintf(temp_str, "ALARM LIMIT:%d"
                      "\xB0",
            tUI_PuSetting.ubTempMax);
...
//------------------------------------------------------------------------------
static const uint32_t g_uiBacklightLevels[] = {0x110, 0x500, 0xA00};

static uint8_t UI_NormalizeBacklightIndex(uint8_t brightness)
{
    uint8_t levelCount = (uint8_t)(sizeof(g_uiBacklightLevels) / sizeof(g_uiBacklightLevels[0]));
    if (brightness < 1 || brightness > levelCount) {
        brightness = 1;
    }
    return brightness;
}

void UI_UpdateBacklight(void)
{
    uint8_t brightness = UI_NormalizeBacklightIndex(tUI_PuSetting.ubLcdBrightness);
...
void UI_DrawString(const char *str, uint16_t startX, uint16_t startY)
{
    ...
    for (i = 0; str[i] != '\0'; ++i) {
        ...
        switch (character) {
            case 'A':
                imageIndex = OSD2IMG_N80_A;
                break;
            case 'B':
                imageIndex = OSD2IMG_N80_B;
                break;
            case 'C':
                imageIndex = OSD2IMG_N80_C;
                break;
            case 'D':
                imageIndex = OSD2IMG_N80_D;
                break;
            case 'E':
                imageIndex = OSD2IMG_N80_E;
                break;
            case 'F':
                imageIndex = OSD2IMG_N80_F;
                break;
            case 'G':
                imageIndex = OSD2IMG_N80_G;
                break;
            case 'H':
                imageIndex = OSD2IMG_N80_H;
                break;
            case 'I':
                imageIndex = OSD2IMG_N80_I;
                break;
            case 'J':
                imageIndex = OSD2IMG_N80_J;
                break;
            case 'K':
                imageIndex = OSD2IMG_N80_K;
                break;
            case 'L':
                imageIndex = OSD2IMG_N80_L;
                break;
            case 'M':
                imageIndex = OSD2IMG_N80_M;
                break;
            case 'N':
                imageIndex = OSD2IMG_N80_N;
                break;
            case 'O':
                imageIndex = OSD2IMG_N80_O;
                break;
            case 'P':
                imageIndex = OSD2IMG_N80_P;
                break;
            case 'Q':
                imageIndex = OSD2IMG_N80_Q;
                break;
            case 'R':
                imageIndex = OSD2IMG_N80_R;
                break;
            case 'S':
                imageIndex = OSD2IMG_N80_S;
                break;
            case 'T':
                imageIndex = OSD2IMG_N80_T;
                break;
            case 'U':
                imageIndex = OSD2IMG_N80_U;
                break;
            case 'V':
                imageIndex = OSD2IMG_N80_V;
                break;
            case 'W':
                imageIndex = OSD2IMG_N80_W;
                break;
            case 'X':
                imageIndex = OSD2IMG_N80_X;
                break;
            case 'Y':
                imageIndex = OSD2IMG_N80_Y;
                break;
            case 'Z':
                imageIndex = OSD2IMG_N80_Z;
                break;
            case ' ':
                imageIndex = OSD2IMG_SPACE1;
                break;
            case '0':
                imageIndex = OSD2IMG_N80_NUM0;
                break;
            case '1':
                imageIndex = OSD2IMG_N80_NUM1;
                break;
            case '2':
                imageIndex = OSD2IMG_N80_NUM2;
                break;
            case '3':
                imageIndex = OSD2IMG_N80_NUM3;
                break;
            case '4':
                imageIndex = OSD2IMG_N80_NUM4;
                break;
            case '5':
                imageIndex = OSD2IMG_N80_NUM5;
                break;
            case '6':
                imageIndex = OSD2IMG_N80_NUM6;
                break;
            case '7':
                imageIndex = OSD2IMG_N80_NUM7;
                break;
            case '8':
                imageIndex = OSD2IMG_N80_NUM8;
                break;
            case '9':
                imageIndex = OSD2IMG_N80_NUM9;
                break;
            case '%':
                imageIndex = OSD2IMG_PERCENT31;
                break;
            case '-':
                imageIndex = OSD2IMG_NEGATIVE31;
                break;
            case '/':
                imageIndex = OSD2IMG_BACKSLASH31;
                break;
            case ':':
                imageIndex = OSD2IMG_COLON31;
                break;
            case 0xB0:
                imageIndex = OSD2IMG_TEMP31;
                break;
            default:
                continue;
        }
...
void UI_DrawReverseString23(const char *str, uint16_t startX, uint16_t startY)
{
    ...
    for (i = 0; str[i] != '\0'; ++i) {
        ...
        switch (character) {
            case 'A':
                imageIndex = OSD2IMG_R23_A;
                break;
            case 'B':
                imageIndex = OSD2IMG_R23_B;
                break;
            case 'C':
                imageIndex = OSD2IMG_R23_C;
                break;
            case 'D':
                imageIndex = OSD2IMG_R23_D;
                break;
            case 'E':
                imageIndex = OSD2IMG_R23_E;
                break;
            case 'F':
                imageIndex = OSD2IMG_R23_F;
                break;
            case 'G':
                imageIndex = OSD2IMG_R23_G;
                break;
            case 'H':
                imageIndex = OSD2IMG_R23_H;
                break;
            case 'I':
                imageIndex = OSD2IMG_R23_I;
                break;
            case 'J':
                imageIndex = OSD2IMG_R23_J;
                break;
            case 'K':
                imageIndex = OSD2IMG_R23_K;
                break;
            case 'L':
                imageIndex = OSD2IMG_R23_L;
                break;
            case 'M':
                imageIndex = OSD2IMG_R23_M;
                break;
            case 'N':
                imageIndex = OSD2IMG_R23_N;
                break;
            case 'O':
                imageIndex = OSD2IMG_R23_O;
                break;
            case 'P':
                imageIndex = OSD2IMG_R23_P;
                break;
            case 'Q':
                imageIndex = OSD2IMG_R23_Q;
                break;
            case 'R':
                imageIndex = OSD2IMG_R23_R;
                break;
            case 'S':
                imageIndex = OSD2IMG_R23_S;
                break;
            case 'T':
                imageIndex = OSD2IMG_R23_T;
                break;
            case 'U':
                imageIndex = OSD2IMG_R23_U;
                break;
            case 'V':
                imageIndex = OSD2IMG_R23_V;
                break;
            case 'W':
                imageIndex = OSD2IMG_R23_W;
                break;
            case 'X':
                imageIndex = OSD2IMG_R23_X;
                break;
            case 'Y':
                imageIndex = OSD2IMG_R23_Y;
                break;
            case 'Z':
                imageIndex = OSD2IMG_R23_Z;
                break;
            case ' ':
                imageIndex = OSD2IMG_SPACE3;
                break;
            case '0':
                imageIndex = OSD2IMG_R23_NUM0;
                break;
            case '1':
                imageIndex = OSD2IMG_R23_NUM1;
                break;
            case '2':
                imageIndex = OSD2IMG_R23_NUM2;
                break;
            case '3':
                imageIndex = OSD2IMG_R23_NUM3;
                break;
            case '4':
                imageIndex = OSD2IMG_R23_NUM4;
                break;
            case '5':
                imageIndex = OSD2IMG_R23_NUM5;
                break;
            case '6':
                imageIndex = OSD2IMG_R23_NUM6;
                break;
            case '7':
                imageIndex = OSD2IMG_R23_NUM7;
                break;
            case '8':
                imageIndex = OSD2IMG_R23_NUM8;
                break;
            case '9':
                imageIndex = OSD2IMG_R23_NUM9;
                break;
            case '%':
                imageIndex = OSD2IMG_PERCENTR23;
                break;
            case '-':
                imageIndex = OSD2IMG_NEGATIVER23;
                break;
            case '/':
                imageIndex = OSD2IMG_BACKSLASHR23;
                break;
            case ':':
                imageIndex = OSD2IMG_COLONR23;
                break;
            case 0xB0:
                imageIndex = OSD2IMG_TEMPR23;
                break;
            default:
                continue;
        }
...
void UI_DrawString23(const char *str, uint16_t startX, uint16_t startY)
{
    ...
    for (i = 0; str[i] != '\0'; ++i) {
        ...
        switch (character) {
            case 'A':
                imageIndex = OSD2IMG_PX23_A;
                break;
            case 'B':
                imageIndex = OSD2IMG_PX23_B;
                break;
            case 'C':
                imageIndex = OSD2IMG_PX23_C;
                break;
            case 'D':
                imageIndex = OSD2IMG_PX23_D;
                break;
            case 'E':
                imageIndex = OSD2IMG_PX23_E;
                break;
            case 'F':
                imageIndex = OSD2IMG_PX23_F;
                break;
            case 'G':
                imageIndex = OSD2IMG_PX23_G;
                break;
            case 'H':
                imageIndex = OSD2IMG_PX23_H;
                break;
            case 'I':
                imageIndex = OSD2IMG_PX23_I;
                break;
            case 'J':
                imageIndex = OSD2IMG_PX23_J;
                break;
            case 'K':
                imageIndex = OSD2IMG_PX23_K;
                break;
            case 'L':
                imageIndex = OSD2IMG_PX23_L;
                break;
            case 'M':
                imageIndex = OSD2IMG_PX23_M;
                break;
            case 'N':
                imageIndex = OSD2IMG_PX23_N;
                break;
            case 'O':
                imageIndex = OSD2IMG_PX23_O;
                break;
            case 'P':
                imageIndex = OSD2IMG_PX23_P;
                break;
            case 'Q':
                imageIndex = OSD2IMG_PX23_Q;
                break;
            case 'R':
                imageIndex = OSD2IMG_PX23_R;
                break;
            case 'S':
                imageIndex = OSD2IMG_PX23_S;
                break;
            case 'T':
                imageIndex = OSD2IMG_PX23_T;
                break;
            case 'U':
                imageIndex = OSD2IMG_PX23_U;
                break;
            case 'V':
                imageIndex = OSD2IMG_PX23_V;
                break;
            case 'W':
                imageIndex = OSD2IMG_PX23_W;
                break;
            case 'X':
                imageIndex = OSD2IMG_PX23_X;
                break;
            case 'Y':
                imageIndex = OSD2IMG_PX23_Y;
                break;
            case 'Z':
                imageIndex = OSD2IMG_PX23_Z;
                break;
            case ' ':
                imageIndex = OSD2IMG_SPACE2;
                break;
            case '0':
                imageIndex = OSD2IMG_PX23_NUM0;
                break;
            case '1':
                imageIndex = OSD2IMG_PX23_NUM1;
                break;
            case '2':
                imageIndex = OSD2IMG_PX23_NUM2;
                break;
            case '3':
                imageIndex = OSD2IMG_PX23_NUM3;
                break;
            case '4':
                imageIndex = OSD2IMG_PX23_NUM4;
                break;
            case '5':
                imageIndex = OSD2IMG_PX23_NUM5;
                break;
            case '6':
                imageIndex = OSD2IMG_PX23_NUM6;
                break;
            case '7':
                imageIndex = OSD2IMG_PX23_NUM7;
                break;
            case '8':
                imageIndex = OSD2IMG_PX23_NUM8;
                break;
            case '9':
                imageIndex = OSD2IMG_PX23_NUM9;
                break;
            case '%':
                imageIndex = OSD2IMG_PERCENT23;
                break;
            case '-':
                imageIndex = OSD2IMG_NEGATIVE23;
                break;
            case '.':
                imageIndex = OSD2IMG_PERIOD23;
                break;
            case ',':
                imageIndex = OSD2IMG_COMMA23;
                break;
            case '?':
                imageIndex = OSD2IMG_QUESTION23;
                break;
            case '(':
                imageIndex = OSD2IMG_BRACKETLEFT23;
                break;
            case ')':
                imageIndex = OSD2IMG_BRACKETRIGHT23;
                break;
            case '/':
                imageIndex = OSD2IMG_BACKSLASH23;
                break;
            case ':':
                imageIndex = OSD2IMG_COLON23;
                break;
            case 0xB0:
                imageIndex = OSD2IMG_TEMP23;
                break;
            case 0xC6:
                imageIndex = OSD2IMG_AE23;
                break;
            case 0xD8:
                imageIndex = OSD2IMG_O_SLASH23;
                break;
            case 0xC5:
                imageIndex = OSD2IMG_A_RING23;
                break;
            default:
                continue;
        }
...
//------------------------------------------------------------------------------
static inline void UI_KbdCursorFromIndex(uint8_t idx)
{
    if (idx < KEYBOARD_ROW_0_COLS) {
        g_rectangle_x = 40 + idx * 40;
        g_rectangle_y = 160;
    } else if (idx < KEYBOARD_ROW_0_COLS + KEYBOARD_ROW_1_COLS) {
        uint8_t i     = idx - KEYBOARD_ROW_0_COLS;
        g_rectangle_x = 40 + i * 40;
        g_rectangle_y = 200;
    } else if (idx < KEYBOARD_ROW_0_COLS + KEYBOARD_ROW_1_COLS + 4) {
        uint8_t i     = idx - (KEYBOARD_ROW_0_COLS + KEYBOARD_ROW_1_COLS);
        g_rectangle_x = 40 + i * 40;
        g_rectangle_y = 240;
    } else if (idx < TOTAL_KEYBOARD_KEYS) {
        uint8_t i     = idx - (KEYBOARD_ROW_0_COLS + KEYBOARD_ROW_1_COLS + 4);
        g_rectangle_x = 40 * 5 + i * 100;
        g_rectangle_y = 240;
    } else if (idx == TOTAL_KEYBOARD_KEYS) {
        // SET AS NEW NAME
        g_rectangle_x = 40;
        g_rectangle_y = 305;
    } else { // idx == TOTAL_KEYBOARD_KEYS + 1
        // EXIT
        g_rectangle_x = 40;
        g_rectangle_y = 345;
    }
...
void MoveboxDown(void)
{
    ...
    switch (g_current_menu_level) {
        ...
        case MENU_CHANGE_NAME: {
            ...
            UI_KbdCursorFromIndex(g_current_menu_index);
            break;
        }
        case MENU_CHANGE_PRIORITY_MAIN:
            UI_UpdateChangePriorityPositions();
            g_current_menu_index = (g_current_menu_index + 1) % PRIORITY_MENU_ITEM_COUNT;
            g_rectangle_y        = PRIORITY_MENU_Y_POS[g_current_menu_index];
            break;
        ...
        case MENU_PAIR_MODE:
            g_current_menu_index = (g_current_menu_index + 1) % PAIRMODE_MENU_ITEM_COUNT;
            g_rectangle_y        = PAIRMODE_MENU_Y_POS[g_current_menu_index];
            break;

        case MENU_PAIR_MODE_GUIDE:
            g_current_menu_index = (g_current_menu_index + 1) % PAIRMODE_GUIDE_ITEM_COUNT;
            g_rectangle_y        = PAIRMODE_GUIDE_Y_POS[g_current_menu_index];
            break;

        case MENU_UNPAIR_UNITS:
            UI_UpdateUnpairMenuPositions();
            g_current_menu_index = (g_current_menu_index + 1) % UNPAIR_MENU_ITEM_COUNT;
            g_rectangle_y        = UNPAIR_MENU_Y_POS[g_current_menu_index];
            break;

        case MENU_UNPAIR_BABY_UNIT:
            g_current_menu_index = (g_current_menu_index + 1) % 2;
            g_rectangle_y        = (g_current_menu_index == 0) ? 160 : 200;
            break;
...
void MoveboxUp(void)
{
    ...
    switch (g_current_menu_level) {
        ...
        case MENU_CHANGE_PRIORITY_MAIN:
            UI_UpdateChangePriorityPositions();
            g_current_menu_index = (g_current_menu_index + PRIORITY_MENU_ITEM_COUNT - 1) % PRIORITY_MENU_ITEM_COUNT;
            g_rectangle_y        = PRIORITY_MENU_Y_POS[g_current_menu_index];
            break;
        ...
        case MENU_PAIR_MODE:
            g_current_menu_index = (g_current_menu_index + PAIRMODE_MENU_ITEM_COUNT - 1) % PAIRMODE_MENU_ITEM_COUNT;
            g_rectangle_y        = PAIRMODE_MENU_Y_POS[g_current_menu_index];
            break;

        case MENU_PAIR_MODE_GUIDE:
            g_current_menu_index = (g_current_menu_index + PAIRMODE_GUIDE_ITEM_COUNT - 1) % PAIRMODE_GUIDE_ITEM_COUNT;
            g_rectangle_y        = PAIRMODE_GUIDE_Y_POS[g_current_menu_index];
            break;

        case MENU_UNPAIR_UNITS:
            UI_UpdateUnpairMenuPositions();
            g_current_menu_index =
                (g_current_menu_index + UNPAIR_MENU_ITEM_COUNT - 1) % UNPAIR_MENU_ITEM_COUNT;
            g_rectangle_y = UNPAIR_MENU_Y_POS[g_current_menu_index];
            break;
    ...
    switch (g_current_menu_level) {
        case 0:
            ...
            } else // ok_butten
            {
                ...
                {
                    case 1:
                        g_rectangle_y = KEYLOCK_MENU_Y_POS[0];
                        break;
                    case 2:
                        g_rectangle_y = ZOOM_MENU_Y_POS[0];
                        break;
                    case 4:
                        g_rectangle_y = SETTINGS_MENU_Y_POS[0];
                        break;
                    case 5: // PAIR UNITS
                    {
                        uint8_t connected_count = 0;
                    ...
                    case 6:
                        g_rectangle_y = INFO_MENU_Y_POS[0];
                        break;
                }
            }
            break;
        ...
        case MENU_LEVEL_SLEEP_TIMER:
            switch (g_current_menu_index) {
                case 0: // ON/OFF
                    ...
                    {
                        for (int i = 0; i < CAM_4T; i++) {
                            if (g_sleep_timer_start_ms[i] > 0) { // timer running
                ...
                case 1:                     // STOP
                    ...
                    for (int i = 0; i < CAM_4T; i++) {
                        if (tUI_CamStatus[i].sleep_time_ms > 0) {
                            UI_SleepHist_Push(i, tUI_CamStatus[i].sleep_time_ms);
        ...
        case MENU_SLEEP_HISTORY_OPTIONS:
            if (g_current_menu_index == 0) {
                ...
                g_settings_changed = 1;
								g_current_menu_level = MENU_SLEEP_HISTORY_DELETED;
								g_current_menu_index = 0;
								g_rectangle_x        = 40;
								g_rectangle_y        = 150;
            } else {
...
case MENU_SLEEP_HISTORY_DELETED:    
    ...
    break;
        ...
        case MENU_DISPLAYSETTINGS:
            switch (g_current_menu_index) {
                case 0: // LCD BRIGHTNESS
                    ...
                    tUI_PuSetting.ubLcdBrightness = UI_NormalizeBacklightIndex(tUI_PuSetting.ubLcdBrightness + 1);
                    UI_UpdateBacklight();
                    g_settings_changed = 1;
                    break;
        ...
        case MENU_ADVANCEDDISPLAYSETTINGS:
            switch (g_current_menu_index) {
                case 0:
                    tUI_PuSetting.ubTextTransparency++;
                    if (tUI_PuSetting.ubTextTransparency > 3) {
                        tUI_PuSetting.ubTextTransparency = 0;
                    }
        ...
        case MENU_BABYUNITSETTINGS: {
            ...
            } else if (g_current_menu_index == bu_count) // PAIR_GetPairedDevCnt(),tUI_PuSetting.ubPairedBuNum
            {
                // CHANGE PRIORITY
                UI_UpdateChangePriorityPositions();
                g_current_menu_level = MENU_CHANGE_PRIORITY_MAIN;
                g_current_menu_index = 0;
                g_rectangle_x        = 40;
                g_rectangle_y        = PRIORITY_MENU_Y_POS[0];
            ...
            if (ubTRX_GetLinkStatus(g_selected_bu_index)) {
                switch (g_current_menu_index) {
                    case 0: // ACTIVE/INACTIVE
												tUI_CamStatus[g_selected_bu_index].active++;
												if(tUI_CamStatus[g_selected_bu_index].active > 1)
												{
														tUI_CamStatus[g_selected_bu_index].active = 0;
												}
												g_settings_changed = 1;
                        break;
        ...
        case MENU_CHANGE_NAME: {
            ...
            if (g_current_menu_index < KEYBOARD_ROW_0_COLS) {
                selected = ENGLISH_KEYBOARD[0][g_current_menu_index];
            } else if (g_current_menu_index < KEYBOARD_ROW_0_COLS + KEYBOARD_ROW_1_COLS) {
                uint8_t i = g_current_menu_index - KEYBOARD_ROW_0_COLS;
                selected  = ENGLISH_KEYBOARD[1][i];
            } else if (g_current_menu_index < KEYBOARD_ROW_0_COLS + KEYBOARD_ROW_1_COLS + 4) {
                uint8_t i = g_current_menu_index - (KEYBOARD_ROW_0_COLS + KEYBOARD_ROW_1_COLS);
                selected  = ENGLISH_KEYBOARD[2][i];
            } else if (g_current_menu_index < TOTAL_KEYBOARD_KEYS) {
                uint8_t i = g_current_menu_index - (KEYBOARD_ROW_0_COLS + KEYBOARD_ROW_1_COLS + 4);
                selected  = ENGLISH_KEYBOARD[2][4 + i];
            } else if (g_current_menu_index == TOTAL_KEYBOARD_KEYS) {
                // SET AS NEW NAME
                ...
                MenuExitHandler();
                break;
            } else { // g_current_menu_index == TOTAL_KEYBOARD_KEYS + 1
                MenuExitHandler();
                break;
            }
        ...
        case MENU_CHANGE_PRIORITY_MAIN: {
            ...
            g_pri_msg_show = false;
            uint8_t online = 0;
            for (uint8_t i = 0; i < CAM_4T; ++i)
                if (ubTRX_GetLinkStatus(i)) online++;
            ...
            if (g_current_menu_index == 0) {
                g_pri_step     = 1;
                g_pri_order[0] = g_pri_order[1] = g_pri_order[2] = 0xFF;
                g_pri_mask                                       = 0;
                g_current_menu_level                             = MENU_CHANGE_PRIORITY_PICK;
                g_current_menu_index                             = 0;
                UI_UpdateChangePriorityPositions();
                g_rectangle_x = 40;
            } else {
        ...
        case MENU_CHANGE_PRIORITY_PICK: {
            ...
            if (g_current_menu_index == n) {
                g_current_menu_level = MENU_CHANGE_PRIORITY_MAIN;
                g_current_menu_index = 0;
                break;
            }

            uint8_t chosen_ch           = avail[g_current_menu_index];
            g_pri_order[g_pri_step - 1] = chosen_ch;
            g_pri_mask |= (1u << chosen_ch);
            g_pri_step++;

            uint8_t online = 0;
            for (uint8_t i = 0; i < 3; ++i) {
                uint8_t ch = g_bu_idx_map[i];
                if (ubTRX_GetLinkStatus(ch)) online++;
            }

            if (g_pri_step <= online) {
                g_current_menu_index = 0;
                UI_UpdateChangePriorityPositions();
            } else {
                for (uint8_t rank = 0; rank < online; ++rank) {
                    uint8_t ch = g_pri_order[rank];
                    if (ch != 0xFF) {
                        tUI_CamStatus[ch].priority = rank;
                    }
                }
        ...
        case MENU_PAIR_NEW_BABY_UNIT:
            switch (g_current_menu_index) {
                case 0:
                    ...
                    g_current_menu_index = 0;
                    g_rectangle_x        = 40;
                    g_rectangle_y        = PAIRMODE_MENU_Y_POS[0];
                    break;
                case 1:
                    MenuExitHandler();
                    break;
        ...
        case MENU_PAIR_MODE:
            switch (g_current_menu_index) {
                case 0: // 1
                    g_pair_target_cnt    = 1;
                    g_current_menu_level = MENU_PAIR_MODE_GUIDE;
                    g_current_menu_index = 0;
                    g_rectangle_x        = 40;
                    g_rectangle_y        = PAIRMODE_GUIDE_Y_POS[0];
                    break;
                case 1: // 2
                    g_pair_target_cnt    = 2;
                    g_current_menu_level = MENU_PAIR_MODE_GUIDE;
        ...
        case MENU_PAIR_MODE_GUIDE:
            if (g_current_menu_index == 0) {       // CONTINUE
                g_pair_needed = g_pair_target_cnt; // 1 or 2 or 3
                if (g_pair_needed > 3) g_pair_needed = 3;
        ...
        case MENU_UNPAIR_UNITS: {
            uint8_t bu_cnt = 0;
            for (uint8_t i = 0; i < 3; ++i)
                if (ubTRX_GetLinkStatus(i)) bu_cnt++;

            if (g_current_menu_index == (UNPAIR_MENU_ITEM_COUNT - 1) || bu_cnt == 0) {
                g_current_menu_level = 5;
                g_current_menu_index = (bu_cnt >= 3) ? 0 : 1;
                UI_UpdatePairUnitsMenuPositions();
                g_rectangle_y = PAIRUNITS_MENU_Y_POS[g_current_menu_index];
                break;
            }

            g_unpair_pick_menu_idx = g_current_menu_index;
            g_unpair_target_ch     = g_bu_idx_map[g_unpair_pick_menu_idx];

            g_current_menu_level = MENU_UNPAIR_BABY_UNIT;
            g_current_menu_index = 0; // YES
            g_rectangle_x        = 40;
            g_rectangle_y        = 160;
            break;
        }
...
{
    if (tUI_PuSetting.ubTextTransparency < 4) {
        OSD_Weight(transparency_levels[tUI_PuSetting.ubTextTransparency]);
    }
    // CLEAN
    OSD_ClearImg1Buf();
