...
LCD Display FlowChart:
	\dot
digraph "LCD Display Flow" 
{
		node [shape=record, fontname=Verdana, fontsize=10, fixedsize=true, width=2];
		edge [fontname=Verdana];
		"Image and Setting Ready" [shape=polygon, sides=9,color=blue, fontcolor=blue] ;
		...
		tLCD_CropScale -> LCD_JpegRwQTab [fontcolor=blue, fontsize=8, label="Jpeg 422"]; 
		LCD_JpegRwQTab -> tLCD_JpegDecodeEnable -> "Image and Setting Ready" [fontcolor=blue, fontsize=8, label="Jpeg 422 Setting Ready"];
		tLCD_JpegDecodeEnable -> "No Video" [color=red, fontcolor=red, fontsize=8, tailport=w, label="Fail", style = dashed];		
		tLCD_CropScale -> "Image and Setting Ready" [fontcolor=blue, fontsize=8, tailport=se, headport=n, label="Raw Setting Ready"];
		ulLCD_CalLcdBufSize -> LCD_SetLcdBufAddr [fontcolor=blue, fontsize=12, label="Assign LCD Buffer"];
		LCD_SetLcdBufAddr -> pLCD_GetLcdChBufInfor -> LCD_SetChBufReady [fontcolor=blue, fontsize=8, label="CHx"];
		LCD_SetChBufReady ->"Image and Setting Ready"  [fontcolor=blue, fontsize=8, label="Image Ready"];
		"Image and Setting Ready" -> LCD_ChEnable [fontcolor=blue, fontsize=8, label="CHx"];
		LCD_ChEnable -> LCD_Start;
		LCD_ChEnable -> tLCD_WinSft;
		tLCD_WinSft  -> LCD_Start [headport=ne];
		tLCD_WinSft -> "Window No Shift"  [color=red, fontcolor=red, fontsize=8, headport=n, tailport=se, label="Fail", style = dashed]; 
		...
		LCD_Init ->LCD_UnInit[dir=back, headport=e, tailport=e];
}
	\enddot
LCD Change Image:	
	\dot
digraph "LCD Change Image" 
{
		node [shape=record, fontname=Verdana, fontsize=10, fixedsize=true, width=2];
		edge [fontname=Verdana];
		Displaying [shape=polygon, sides=9,color=blue, fontcolor=blue] ;
		"Buffer Empty" [shape=polygon, sides=9,color=blue, fontcolor=blue] ;		
		"Buffer Full" [shape=polygon, sides=9,color=red, fontcolor=red] ;
		"Get Buffer Address" [shape=polygon, sides=9,color=blue, fontcolor=blue] ;
		Displaying -> pLCD_GetLcdChBufInfor [tailport=sw, headport=n, fontcolor=blue, fontsize=8, label="Change Image"];
		Displaying -> tLCD_DynamicOneChCropScale [tailport=se, headport=nw, fontcolor=blue, fontsize=8, label="Change Crop or Scale"];
		tLCD_DynamicOneChCropScale -> Displaying [tailport=ne, headport=e, fontcolor=blue, fontsize=8, label="Wait"];
		pLCD_GetLcdChBufInfor ->"Buffer Empty" [tailport=sw];
		pLCD_GetLcdChBufInfor ->"Buffer Full" [tailport=s, headport=nw,color=red];
		"Buffer Full" -> pLCD_GetLcdChBufInfor [tailport=n, headport=se,color=blue, fontcolor=blue, fontsize=8, label="Wait"];
		"Buffer Full" -> "Get Buffer Address" [headport=e,color=red, fontcolor=red, fontsize=8, label="Skip Last Frame"];
		"Buffer Empty" -> "Get Buffer Address"
		"Get Buffer Address" -> LCD_SetChBufReady [fontcolor=blue, fontsize=8, label="Image"];
		LCD_SetChBufReady -> Displaying [tailport=w, headport=w, fontcolor=blue, fontsize=8, label="Change Next Image"];
}
	\enddot
*/
//------------------------------------------------------------------------------
#if (LCD_PANEL <= LCD_GPM1125A0_BT656)
	#include "LCD_GPM1125A0.h"
#endif
#if (LCD_PANEL >= LCD_TM035KDH03_24HV && LCD_PANEL <= LCD_TM035KDH03_BT656)
	#include "LCD_TM035KDH03.h"
#endif
#if (LCD_PANEL == LCD_SN9C271A_YUV422)
...
#endif
#if (LCD_PANEL == LCD_TC358778_Y50019N00N || LCD_PANEL == LCD_TC358778_Y50019N00N_NEW)
	#include "LCD_TC358778.h"
#endif
#if (LCD_PANEL >= LCD_TM024HDH03_8080_8Bit2Byte && LCD_PANEL <= LCD_TM024HDH03_8080_9Bit)
	#include "LCD_TM024HDH03.h"
#endif
#if (LCD_PANEL >= LCD_TM023KDH03_8080_8Bit2Byte && LCD_PANEL <= LCD_TM023KDH03_8080_16Bit)
	#include "LCD_TM023KDH03.h"
#endif
#if (LCD_PANEL == LCD_AHD_TP2915_RGB24)
	#include "LCD_AHD_TP2915.h"
#endif
...
#endif
#if (LCD_PANEL == LCD_YN700WS008B)
	#include "LCD_YN700WS008B.h"
#endif
#if (LCD_PANEL == LCD_H5024A)
	#include "LCD_H5024A.h"
#endif
#if (LCD_PANEL == LCD_FTD50B7)
	#include "LCD_FTD50B7.h"
#endif
#if (LCD_PANEL == LCD_MG350BN001)
	#include "LCD_MG350BN001.h"
#endif
//------------------------------------------------------------------------------
/*!
	\brief LCD Macro Define
*/
...
//------------------------------------------------------------------------------
#define LCDMAX(a,b)					((a)>(b))?a:b	//!< Max macro define for LCD.c
#define LCDMIN(a,b)					((a)<(b))?a:b	//!< Min macro define for LCD.c
//------------------------------------------------------------------------------
#define LCD_MODE_QUEUE_SZ			(1)				//!< Dynamic switch HDMI output and display pannel for example 
//------------------------------------------------------------------------------
#define LCD_LPF_MAX_AREA			(1904 * 1072)	//!< The max area when LFP enable for timing limit
//------------------------------------------------------------------------------
#ifdef _510PF_SDK_
//! The Mutex for LCD
...
//------------------------------------------------------------------------------
/*!
	\brief Enable / Disable LCD Function
*/
#define LCD_FuncEnable				(GLB->LCD_FUNC_DIS = 0)	//!< Enable  LCD function clock
#define LCD_FuncDisable				(GLB->LCD_FUNC_DIS = 1)	//!< Disable LCD function clock
//------------------------------------------------------------------------------
/*!
...
typedef enum
{
	...
	LCD_DISP_3TC_SHOW_121,					//!< Display three channels, column list(Ratio 1:2:1)
	LCD_DISP_3TC_SHOW_111,					//!< Display three channels, column list(Ratio 1:1:1)
...
//------------------------------------------------------------------------------
/*!
	\brief LCD Channel States
*/
typedef enum
{
	LCD_CH_UNREADY,							//!< LCD Channel UnReady
	LCD_CH_READY,							//!< LCD Channek Ready
}LCD_CH_STA_TYP;
//------------------------------------------------------------------------------
/*!
...
//------------------------------------------------------------------------------
/*!
	\brief LCD Jpeg Enable / Disable Status
*/
typedef enum
{
	LCD_JPEG_DISABLE,
...
//------------------------------------------------------------------------------
/*!
	\brief LCD Interrupt Mask State
*/
typedef enum
{
	LCD_INT_MASK_DISABLE,
	LCD_INT_MASK_ENABLE,
}LCD_INT_MASK_STATE;
//------------------------------------------------------------------------------
/*!
...
//------------------------------------------------------------------------------
/*!
	\brief Calculate LCD Channel Buffer Size
*/
typedef struct
{
...
//------------------------------------------------------------------------------
/*!
	\brief Calculate LCD Buffer Size
*/
typedef struct
{
...
//------------------------------------------------------------------------------
/*!
	\brief 		Get Fixed LCD Channel Buffer Information Function
	\param[in]	tCh Selet Nth channel (LCD_CH0~3)
	\param[in]	ubIdx Selet Buffer Index(0-1)
	\return     LCD Nth channel index buffer information, include buffer address and state
	\par [Example]
	\code    
		 LCD_BUF_TYP *pLcdCh0Buf;

		 pLcdCh0Buf = pLCD_ForcetoGetLcdChBuf(LCD_CH0,0);
	\endcode
*/
LCD_BUF_TYP *pLCD_ForcetoGetLcdChBuf(LCD_CH_TYP tCh,uint8_t ubIdx);
//------------------------------------------------------------------------------
/*!
	\brief 		Check LCD Channel Ready Function
	\param[in]	tCh Selet Nth channel (LCD_CH0~3)
	\return     LCD Nth channel LCD_CH_READY or LCD_CH_UNREADY
	\par [Example]
	\code    
		 tLCD_ChkChReady(LCD_CH0);
	\endcode
*/
LCD_CH_STA_TYP tLCD_ChkChReady (LCD_CH_TYP tCh);
//------------------------------------------------------------------------------
/*!
...
//------------------------------------------------------------------------------
/*!
	\brief 		Set channel color for disable LCD channel.
	\param[in]	ubRed Red Color
	\param[in]	ubGreen Green Color
	\param[in]	ubBlue Blue Color
	\par [Example]
	\code
		 LCD_ChDisableColor(0, 0, 0);		//! Black
		 LCD_ChDisableColor(0, 0, 255);		//! Blue
	\endcode
*/
void LCD_ChDisableColor(uint8_t ubRed, uint8_t ubGreen, uint8_t ubBlue);
//------------------------------------------------------------------------------
/*!
...
/*!
	...
	\endcode
*/
void LCD_IntMask (LCD_INT_MASK_STATE tState);
//osMessageQId* pLCD_SwitchModeInit(uint32_t ulStackSize, osPriority priority);

typedef void(*pvLCD_VsyncNtyCb)(void);
void LCD_SetVsyncNtyCbFunc(pvLCD_VsyncNtyCb cb);
