...
#define LCD_MINORVER   16        //!< Minor version
//------------------------------------------------------------------------------
...
osMutexId osLCD_IntMaskMutex;
osMutexDef(osLCD_Mutex);
#if (LCD_AHD_TP2915_RGB24 == LCD_PANEL)	
static uint32_t ulLCD_IsrCnt;
#endif
pvLCD_VsyncNtyCb pLCD_VsynNty = NULL;
//------------------------------------------------------------------------------
...
{
#if (LCD_AHD_TP2915_RGB24 == LCD_PANEL)	
	static uint8_t ubScore = 3;
#endif
	uint8_t ubi;

	LCD_IntMask(LCD_INT_MASK_ENABLE);    
	ubi = (LCD_BUF_CUR == tLCD_BufInfor.tChBuf[tCh].tBuf[1].tBufState ||
	       LCD_BUF_EMPTY == tLCD_BufInfor.tChBuf[tCh].tBuf[0].tBufState)?0:1;
	if (LCD_BUF_FULL == tLCD_BufInfor.tChBuf[tCh].tBuf[ubi].tBufState)
#if (LCD_AHD_TP2915_RGB24 != LCD_PANEL)		
		tLCD_BufInfor.tChBuf[tCh].tBuf[ubi].tBufState = LCD_BUF_EMPTY;
#else	
	{
		if (LCD->TV_LCD_EN && ulLCD_IsrCnt + ubScore < osKernelSysTick())
		{
			ubi = (ubi)?0:1;
			ubScore = 3;
		}
	...
	while (LCD_BUF_EMPTY != tLCD_BufInfor.tChBuf[tCh].tBuf[ubi].tBufState);
//    {
//        if (LCD_BUF_CUR == tLCD_BufInfor.tChBuf[tCh].tBuf[ubi].tBufState &&
//            LCD_BUF_EMPTY == tLCD_BufInfor.tChBuf[tCh].tBuf[(ubi)?0:1].tBufState)
//        {
//            printf ("LCD buf%d\n",ubi);
//            ubi = (ubi)?0:1;
//            printf ("LCD buf%d error\n", ubi);
//            CAM4_LED_IO(~GET_CAM4_LED_IO);
//            break;
//        }
//    }
	return &tLCD_BufInfor.tChBuf[tCh].tBuf[ubi];
}
//------------------------------------------------------------------------------
LCD_BUF_TYP *pLCD_ForcetoGetLcdChBuf(LCD_CH_TYP tCh,uint8_t ubIdx)
{
	return &tLCD_BufInfor.tChBuf[tCh].tBuf[ubIdx];
}
//------------------------------------------------------------------------------
LCD_CH_STA_TYP tLCD_ChkChReady (LCD_CH_TYP tCh)
{
	return (LCD_BUF_CUR != tLCD_BufInfor.tChBuf[tCh].tBuf[0].tBufState && 
		    LCD_BUF_CUR != tLCD_BufInfor.tChBuf[tCh].tBuf[1].tBufState)?LCD_CH_UNREADY:LCD_CH_READY;
}
//------------------------------------------------------------------------------
uint16_t uwLCD_GetLcdHoSize (void)
{
	return LCD->LCD_HO_SIZE - uwOSD_GetHDummyPixel();
}
//------------------------------------------------------------------------------
...
void LCD_VsyncIsr (void)
{
	...
	for (ubi=0; ubi<ubLCD_MaxChNow; ++ubi)
	{
        ...
        if (LCD_BUF_CUR == tLCD_BufInfor.tChBuf[ubi].tBuf[LCD_PINGPONG_BUF_MAX - ubIndex - 1].tBufState)
...
void LCD_Start (void)
{
	...
	bLCD_IsrEnable = true;
	INTC_IrqSetup(INTC_LCD_IRQ, LCD_VsyncIsr);
	INTC_IrqEnable(INTC_LCD_IRQ);
	LCD->CLR_LCD_INT = 1;
	LCD->LCD_INT_EN  = 1;
	LCD->TV_LCD_EN   = 1;
#if (LCD_PANEL == LCD_SSD2828_Y50019N00N || LCD_PANEL == LCD_SSD2828_YN550FH002N)
	...
	#endif
#endif
#elif (LCD_PANEL == LCD_TC358778_Y50019N00N || LCD_PANEL == LCD_TC358778_Y50019N00N_NEW)
	LCD_MIPI_TC358778_Start();
#elif (LCD_PANEL == LCD_AHD_TP2915_RGB24)
	AHD_TP2915_Start();
#elif (LCD_PANEL == LCD_AHD_NVP6021_RGB24)
	AHD_NVP6021_Start();
#endif
	ubLCD_StartFlag = 1;
}
...
void LCD_ChDisableColor(uint8_t ubRed, uint8_t ubGreen, uint8_t ubBlue)
{
	...
	LCD->DEFAULT_BG_Y  = (fY_Value<0?0:(fY_Value>255?255:fY_Value));
	LCD->DEFAULT_BG_CB = (fCb_Value<0?0:(fCb_Value>255?255:fCb_Value));
	LCD->DEFAULT_BG_CR = (fCr_Value<0?0:(fCr_Value>255?255:fCr_Value));
}
//------------------------------------------------------------------------------
...
void LCD_LcdPanelInit (void)
{
	...
	LCD_HSD070IDW1_Init();
#elif (LCD_YN700WS008B == LCD_PANEL)
	LCD_LVDS_YN700WS008B();
#elif (LCD_PANEL == LCD_AHD_TP2915_RGB24)
	AHD_TP2915_Init();
#elif (LCD_PANEL == LCD_AHD_NVP6021_RGB24)
	LCD_NVP6021_Init();
#elif (LCD_PANEL == LCD_TC358778_Y50019N00N || LCD_PANEL == LCD_TC358778_Y50019N00N_NEW)
	if (true == bLCD_MIPI_TC358778_Init())
		LCD_MIPI_TC358778();		
#elif (LCD_PANEL == LCD_H5024A)
    LCD_H5024A_Init();
#elif (LCD_PANEL == LCD_FTD50B7)
    LCD_FTD50B7_Init();
#elif (LCD_PANEL == LCD_MG350BN001)
    LCD_MG350BN001_Init();	
...
void LCD_Init (LCD_OUTPUT_TYP tLcdOutput)
{
	...
	ubLCD_StartFlag = 0;
	uwLcdH = uwLCD_GetLcdHoSize();
	uwLcdV = uwLCD_GetLcdVoSize();
	if(((uwLcdH >= 1920) || (uwLcdV >= 1920)) &&
	   ((uwLcdV >= 1080) || (uwLcdH >= 1080)))
		DDR_ImprovePriority(SYS_PERMODE3);
	else if(uwLcdH >= 960)
		DDR_ImprovePriority(SYS_PERMODE2);
	else
		DDR_ImprovePriority(SYS_PERMODE1);
    ...
    if(osLCD_IntMaskMutex == NULL) 
	{
        osMutexDef(osLCD_IntMaskMutex);
		osLCD_IntMaskMutex = osMutexCreate(osMutex(osLCD_IntMaskMutex));
		if(osLCD_IntMaskMutex == NULL)
			printf("LCD Init: create interrupt mask mutex fail!\n");
	}
}
//------------------------------------------------------------------------------
void LCD_WinSet(LCD_DISP_TYPE tDispType, uint16_t uwLcdOutputHsize, uint16_t uwLcdOutputVsize)
{
	...
	switch (tDispType)
	{
		...
		case LCD_DISP_3TC_SHOW_121:
			...
			LCD->WND2_Y_START = LCD->WND1_Y_START + (uwLcdOutputVsize >> 1);			
			break;
		case LCD_DISP_3TC_SHOW_111:
			LCD->WND0_X_START = uwWinXStart;
			LCD->WND0_Y_START = uwWinYStart;
			LCD->WND1_X_START = uwWinXStart;			
			LCD->WND1_Y_START = uwWinYStart + (uwLcdOutputVsize/3);
			LCD->WND2_X_START = uwWinXStart;			
			LCD->WND2_Y_START = LCD->WND1_Y_START + (uwLcdOutputVsize/3);			
			break;		
		...
		case LCD_DISP_2T_H12:
			...
			LCD->WND1_Y_START = uwWinYStart;
		#if ( KNL_DISP_ROTATE_SETTING == KNL_DISP_ROTATE_0 )
			//! compensate calc deviation
			if ( LCD->WND1_X_START ) {
				LCD->WND1_X_START -= 1;
			}
		#endif
			...
			if ((pDynInfor->tChRes.uwCropHsize != 0) && (pDynInfor->tChRes.uwCropVsize != 0))
			{
					...
					pDynInfor->tChRes.uwChInputVsize)
				{
					   ...
					   (pDynInfor->tChRes.uwCropVsize << 3) >= pDynInfor->uwLcdOutputVsize)
					{
						...
						tLCD_DynSetting.ubFsHdupEN = (pDynInfor->uwLcdOutputHsize > (tLCD_DynSetting.uwHsize << 4))?1:0;
						tLCD_DynSetting.uwHratio   = 0x200 * tLCD_DynSetting.uwHsize / 
                                                     (pDynInfor->uwLcdOutputHsize >> tLCD_DynSetting.ubFsHdupEN);
						tLCD_DynSetting.uwHoStart  = (((((uint32_t)(0x200 * tLCD_DynSetting.uwHsize / 
...
LCD_RESULT tLCD_CropScale(LCD_INFOR_TYP *pLcdInfor)
{
	...
	for (ubi=0; ubi<pLcdInfor->ubChNum; ++ubi)
	{
		...
		switch (pLcdInfor->tDispType)
		{
			...
			case LCD_DISP_3TC_SHOW_111:
				uwHoSize = pLcdInfor->uwLcdOutputHsize;			
				uwVoSize = pLcdInfor->uwLcdOutputVsize/3;			
				break;						
			case LCD_DISP_3T_2PIP:
				uwHoSize = pLcdInfor->uwLcdOutputHsize >> ((1 == ubi)?0:1);
				uwVoSize = pLcdInfor->uwLcdOutputVsize >> ((1 == ubi)?0:1);
				break;
			case LCD_DISP_2T_H:
				uwHoSize = pLcdInfor->uwLcdOutputHsize >> 1;
		...
		switch (ubi)
		{
			case LCD_CH0:
				...
				LCD->LCD_FS0_HDUP_EN = (uwHoSize > (LCD->DISP0_H_SIZE << 4))?1:0;
				LCD->LCD_FS0_EN = 1;
				LCD->LCD_FS0_HO_SIZE = uwHoSize;
				LCD->LCD_FS0_HRATIO = 0x200 * LCD->DISP0_H_SIZE / (uwHoSize >> LCD->LCD_FS0_HDUP_EN);
				LCD->LCD_FS0_HO_START = (((((uint32_t)(0x200 *  LCD->DISP0_H_SIZE / LCD->LCD_FS0_HRATIO)) << LCD->LCD_FS0_HDUP_EN) - uwHoSize) >> 1) & ~3;
			...
			case LCD_CH1:				
				...
				LCD->LCD_FS1_HDUP_EN = (uwHoSize > (LCD->DISP1_H_SIZE << 4))?1:0;
				LCD->LCD_FS1_EN = 1;
				LCD->LCD_FS1_HO_SIZE = uwHoSize;
				LCD->LCD_FS1_HRATIO = 0x200 * LCD->DISP1_H_SIZE / (uwHoSize >> LCD->LCD_FS1_HDUP_EN);
				LCD->LCD_FS1_HO_START = (((((uint32_t)(0x200 * LCD->DISP1_H_SIZE / LCD->LCD_FS1_HRATIO)) << LCD->LCD_FS1_HDUP_EN) - uwHoSize) >> 1) & ~3;
...
//			ulLCD_ChBufSize[ubi] = ((ulLCD_ChBufSize[ubi] >> 2) + 1) << 2;
	...
	if (LCD->LCD_FS0_FIR_EN || LCD->LCD_FS1_FIR_EN)
	{
		uint32_t ulArea;
		for (ulArea=ubi=0; ubi<pLcdInfor->ubChNum; ++ubi)
			ulArea += pLcdInfor->tChRes[ubi].uwCropHsize * pLcdInfor->tChRes[ubi].uwCropVsize;
		if (LCD_LPF_MAX_AREA < ulArea)
		{
			LCD->LCD_FS0_FIR_EN = 0;
			LCD->LCD_FS1_FIR_EN = 0;
		}
	}
//	for (;ubi<LCD_MAX_CH; ++ubi)
//		ulLCD_ChBufSize[ubi] = 0;
...
{
    static uint16_t	uwIntMaskCnt = 0;
    if(osLCD_IntMaskMutex != NULL)
        osMutexWait(osLCD_IntMaskMutex, osWaitForever);
    if (LCD_INT_MASK_ENABLE == tState)
    {
        if (!uwIntMaskCnt)
            INTC_IrqDisable(INTC_LCD_IRQ);
        ++uwIntMaskCnt;
    }
    else if (uwIntMaskCnt)
    {
        if (1 == uwIntMaskCnt)
            INTC_IrqEnable(INTC_LCD_IRQ);
        --uwIntMaskCnt;
    ...
    if(osLCD_IntMaskMutex != NULL)
        osMutexRelease(osLCD_IntMaskMutex);
}
#if (LCD_HDMI_OUT == LCD_HDMI_IT66121)
