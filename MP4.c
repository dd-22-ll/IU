...
//	DEFINITION                       |
//------------------------------------------------------------------------------
uint8_t ubMP4_LogLvl = MP4_LOG_LV;

#define MP4_GetLogLvl(lv)   ((lv <= ubMP4_LogLvl)? DBG_CriticalLvl : DBG_InfoLvl )
#define MP4_LOGINFO         MP4_GetLogLvl(DBG_InfoLvl)

#define MP4_FORMAT_AAC     1    // Audio stsz=Frame size,stsc samples per chunk=Frame number. 
                                // Windows media player not support.
#define MP4_FORMAT_ALAW    2    // Audio stsz=Fixed 1 byte,stsc samples per chunk=frame size.
#define MP4_FORMAT_MODE    MP4_FORMAT_ALAW
...
FS_MEDIA_SEL MP4_FS_MediaSel = FS_MEDIA_0;
...
#if (MP4_FORMAT_MODE == MP4_FORMAT_AAC)
uint8_t MP4_FtypAtom[MP4_FTYPE_ATOM_SIZE] = {
	0x00, 0x00, 0x00, 0x20,  'f',  't',  'y',  'p',  'i',  's',  'o',  'm', 0x00, 0x00, 0x02, 0x00,
	 'i', 	 's',  'o',  'm',  'i',  's',  'o',  '2',  'a',  'v',  'c',  '1',  'm',  'p',  '4',  '1',
};
#elif (MP4_FORMAT_MODE == MP4_FORMAT_ALAW)
uint8_t MP4_FtypAtom[MP4_FTYPE_ATOM_SIZE] = {
	0x00, 0x00, 0x00, 0x20,  'f',  't',  'y',  'p',  'i',  's',  'o',  'm', 0x00, 0x00, 0x02, 0x00,
	 'i', 	 's',  'o',  'm',  'i',  's',  'o',  '2',  'a',  'v',  'c',  '1',  'a',  'l',  'a',  'w',
};
#endif

// Free Atom, Size = 0x08
uint8_t MP4_FreeAtomInf[8] = {
	0x00, 0x00, 0xFF, 0xD8,  'f',  'r',  'e',  'e',
};

// Free Atom, Size = 472bytes
uint32_t ulMP4_FreeAtom[MP4_SRC_NUM];

// mdat Atom, Size Update finially.
uint8_t MP4_MdatAtom[MP4_MDAT_ATOM_SIZE] = {
...
uint8_t MP4_V1StsdAtom[] = {
	0x00, 0x00, 0x00, 0x89,  's',  't',  's',  'd', 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
};

uint8_t MP4_V1Avc1Atom[] = {
	0x00, 0x00, 0x00, 0x79,  'a',  'v',  'c',  '1', 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x07, 0x80, 0x04, 0x40, 0x00, 0x48, 0x00, 0x00, 0x00, 0x48, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x18, 0xff, 0xff, 0x00, 0x00, 0x00, 0x23,  'a',  'v',  'c',  'C', 0x01, 0x64,
	0x00, 0x29, 0xff, 0xe1, 0x00, 0x0C, 0x67, 0x64, 0x00, 0x29, 0xAD, 0x00, 0x2c, 0xa8, 0x05, 0x00, 0x5B, 0x90,
	0x01, 0x00, 0x04, 0x68, 0xee, 0x31, 0x82,
};
...
#if 1	// Full HD
uint8_t MP4_VSkipFrame[]__attribute__((aligned (32))) = {
	0x00, 0x00, 0x00, 0x42, 0x01, 0x9A, 0x00, 0x01, 0xAF, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00,
...
#if 0	// HD
uint8_t MP4_VSkipFrame[]__attribute__((aligned (32))) = {
	0x00, 0x00, 0x00, 0x20, 0x01, 0x9A, 0x00, 0x01, 0xAF, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00,
	0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x00,
	0x03, 0x00, 0x01, 0x03
};
#endif
...
#if(ADO_ENC_TYPE==SW_AAC_ENC)	// AAC
...
uint8_t MP4_Mp4aAtom[] = {
	...
	0x3E, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x33,  'e',  's',  'd',  's', 0x00, 0x00, 0x00, 0x00,
	0x03, 0x80, 0x80, 0x80, 0x22, 0x00, 0x02, 0x00, 0x04, 0x80, 0x80, 0x80, 0x14, 0x40, 0x15, 0x00,
	0x00, 0x00, 0x00, 0x01, 0xf4, 0x00, 0x00, 0x00, 0x2a, 0x2e, 0x05, 0x80, 0x80, 0x80, 0x02, 0x14,
	0x08, 0x06, 0x80, 0x80, 0x80, 0x01, 0x02,
};
#else   // ALAW
uint8_t MP4_AStsdAtom[] = {
0x00, 0x00, 0x00, 0x4C,  's',  't',  's',  'd', 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
};

uint8_t MP4_Mp4aAtom[] = {
    0x00, 0x00, 0x00, 0x3C, 0x61, 0x6C, 0x61, 0x77, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x10, 0x00, 0x00, 0x00, 0x00,
    0x3E, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x63, 0x68, 0x61, 0x6E, 0x00, 0x00, 0x00, 0x00,
...
FS_WRT_BUF_CNT_MOD_t sHdMod[MP4_SRC_NUM];
uint8_t ubMP4_StreamHD[MP4_SRC_NUM][32];
...
uint8_t ubMP4_V1SpsSize[MP4_SRC_NUM] = {0,0,0,0};
uint8_t ubMP4_V1PpsSize[MP4_SRC_NUM] = {0,0,0,0};
uint8_t ubMP4_V1_Sps[MP4_SRC_NUM][0x10];
uint8_t ubMP4_V1_Pps[MP4_SRC_NUM][0x08];

uint8_t ubMP4_FHD_SPS[0x0C]   = {0x67, 0x64, 0x00, 0x29, 0xAD, 0x00, 0x2C, 0xA8, 0x07, 0x80, 0x22, 0x64};
uint8_t ubMP4_SXGA_SPS[0x0C]  = {0x67, 0x64, 0x00, 0x29, 0xAD, 0x00, 0x2C, 0xA8, 0x05, 0x00, 0x20, 0xE4};
uint8_t ubMP4_HD_SPS[0x0C]    = {0x67, 0x64, 0x00, 0x29, 0xAD, 0x00, 0x2C, 0xA8, 0x05, 0x00, 0x5B, 0x90};
uint8_t ubMP4_XGA_SPS[0x0C]   = {0x67, 0x64, 0x00, 0x29, 0xAD, 0x00, 0x2C, 0xA8, 0x04, 0x00, 0x61, 0x90};
uint8_t ubMP4_WSVGA_SPS[0x0C] = {0x67, 0x64, 0x00, 0x29, 0xAD, 0x00, 0x2C, 0xA8, 0x04, 0x00, 0x4D, 0x90};
uint8_t ubMP4_SVGA_SPS[0x0C]  = {0x67, 0x64, 0x00, 0x29, 0xAD, 0x00, 0x2C, 0xA8, 0x0C, 0x81, 0x36, 0x40};
uint8_t ubMP4_WVGA_SPS[0x0B]  = {0x67, 0x64, 0x00, 0x29, 0xAD, 0x00, 0x2C, 0xA8, 0x0C, 0x83, 0xD9};
uint8_t ubMP4_VGA_SPS[0x0B]   = {0x67, 0x64, 0x00, 0x29, 0xAD, 0x00, 0x2C, 0xA8, 0x0A, 0x03, 0xD9};

uint8_t ubMP4_PPS[0x04]   = {0x68, 0xEE, 0x31, 0x82};
...
uint32_t MP4_AStszAtomSim[MP4_SRC_NUM];
// Video2
...
// Thumbnail
uint32_t ulMP4ThumbnailAdr[MP4_SRC_NUM];
uint32_t ulMP4ThumbnailSize[MP4_SRC_NUM] = {0};

uint32_t ulMP4_FileSize[MP4_SRC_NUM];
uint32_t ulMP4_AtomSize[MP4_SRC_NUM];

uint8_t ubMP4_PIC_TOGGLE[MP4_SRC_NUM] = {0};    // PIC Over flow Toggle
...
// For MP4 Play
uint8_t ubMP4_FormatPly[MP4_SRC_NUM]={0};
...
static void vMP4_CheckUpdateStsc(uint8_t ubCh,uint8_t *pStsc,uint8_t *pStco,uint8_t ubStreamType,uint32_t ulDesID)
{
	...
	if(sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStscSampeDesID != ulDesID)
	{
		...
		printd(MP4_LOGINFO,"Usc %d, %x, %x, %x, %x\r\n",	ubStreamType,
...
static void vMP4_CheckSourceSWUpdateChunk(uint8_t ubCh,uint8_t ubStreamType)
{
	...
	if(ubPreStreamType != ubStreamType)
	{
		...
		{
			//printd(MP4_LOGINFO,"PSC(ST:%d EN:%x FC:%x SPC:%x SSD:%x)\r\n",	ubPreStreamType,
		...
		//printd(MP4_LOGINFO,"PCO(ST:%d EN:%x OF:%x)\r\n",	ubPreStreamType,
...
#if (MP4_FORMAT_MODE == MP4_FORMAT_ALAW)
static void vMP4_AdoUpdateStsc(uint8_t ubCh, uint8_t ubStreamType)
{
...
static void vMP4_AdoCheckUpdateStts(uint8_t ubCh,uint8_t *pStts,uint8_t ubStreamType, uint32_t ulSize,uint32_t ulDuration)
{
	if(sRec_MP4Controller[ubCh].ubChunkStartFlag == 0 )
		return;
    //printd(MP4_LOGINFO,"Ats %d %d %x %d\n",ubCh,ubStreamType,ulSize,ulDuration);
    sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulSttsSampleCount += ulSize;
    sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulSttsSampleDuration = ulDuration;
}

static void vMP4_AdoCheckUpdateStsc(uint8_t ubCh,uint8_t *pStsc,uint8_t *pStco,uint8_t ubStreamType, uint32_t ulSize,uint32_t ulDesID)
{
	if(sRec_MP4Controller[ubCh].ubChunkStartFlag == 0 )
		return;
    ...
    if( sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStscSamplePerChunk == 0 )
    {
        sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStscSamplePerChunk = ulSize;
        sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStscSampeDesID = ulDesID;
        return;
    }
	...
	{
		// Update video stsc atom
		//printd(MP4_LOGINFO,"Asc %d %d, %x, %x, %x\r\n",ubCh ,ubStreamType,
		//									sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStscEntry,
		//									sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStscSamplePerChunk,
		//									sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStscSampeDesID);
		vMP4_SetLong(pStsc, 0, (uint32_t)(16 + 12 * sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStscEntry));
		...
		// Update video stco atom
		//printd(MP4_LOGINFO,"Aco %d %d, %x, %x \r\n", ubCh,	ubStreamType,
		//							sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStcoChunkOffsetEntry,
		//							sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStcoChunkOffset);
	...
	{
	    if(	(sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStscSamplePerChunk != sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStscPreSamplePerChunk) ||
			(sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStscSampeDesID != sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStscPreSampeDesID) )
	    {
	        // Update video stsc atom
	        vMP4_SetLong(pStsc, 0, (uint32_t)(16 + 12 * sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStscEntry));
    		vMP4_SetLong(pStsc, 12, sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStscEntry);
...
static void vMP4_AdoChkSrcSWUpdateChunk(uint8_t ubCh,uint8_t ubStreamType,uint32_t ulSize)
{
	uint8_t ubPreStreamType;
	...
	if(ubPreStreamType != ubStreamType)
	{
			...
			(sRec_MP4Controller[ubCh].stStbl[ubPreStreamType].ulStscSampeDesID != sRec_MP4Controller[ubCh].stStbl[ubPreStreamType].ulStscPreSampeDesID) )
		{
			...
			// Update video stsc atom
			vMP4_AdoUpdateStsc(ubCh, ubPreStreamType);
...
static void vMP4_UpdateAudioAtom(uint8_t ubCh,uint8_t ubStreamType)
{
	...
	if(sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStscSamplePerChunk != 0)
	{
			...
			(sRec_MP4Controller[ubCh].stStbl[sRec_MP4Controller[ubCh].ubStreamType].ulStscSampeDesID != sRec_MP4Controller[ubCh].stStbl[sRec_MP4Controller[ubCh].ubStreamType].ulStscPreSampeDesID) )
		{
			...
			vMP4_UpdateStsc(ubCh, ubStreamType);
#elif(MP4_FORMAT_MODE == MP4_FORMAT_ALAW)
			vMP4_AdoUpdateStsc(ubCh, ubStreamType);
#endif
		}
	}
...
#elif(MP4_FORMAT_MODE == MP4_FORMAT_ALAW)
	vMP4_SetLong((uint8_t *)MP4_AStszAtom[ubCh], 0,  20);
	vMP4_SetLong((uint8_t *)MP4_AStszAtom[ubCh], 12,  1);
#endif
	...
	ulTrackDuration[ubCh][ubStreamType] = ((ulMdhdDuration[ubCh][ubStreamType]*(ulMvhdTimeScale[ubCh]/10))/(ulMdhdTimeScale[ubCh][ubStreamType]/10));
...
uint32_t ulMP4_GetBufSz(void)
{
	...
	ulSize += MP4_STBL_1MINSZ(ubMP4_MaxVFPS,ubMP4_MaxAFPS)*ulMP4_RecTime + MP4_STBL_HDSIZE + MP4_MOOV_HDSIZE;
	
	for(i=0;i<ubMP4_CfgSrcNum;i++)
	{
		ubSourceType = ubMP4_SourceType[i];
		ulSize += MP4_HEAD_SIZE;
    ...
    ulSize += (MP4_SIMSTSZ_SIZE*MP4_SRC_NUM);
	return ulSize;
}

void MP4_GetSpsPpsSize(uint8_t ubCh,uint8_t *pAddr)
{
	uint8_t i,j = 0;
	for( i = 0 ; i < 0x20 ; i++)
	{
        j = 0;
        if((pAddr[i] == 0x01)&&(pAddr[i+1] == 0x67))        //SPS
        {
            do
            {    
                ubMP4_V1_Sps[ubCh][j] = pAddr[i+1];
                j++;          
                i++;
            }while(!((pAddr[i+1] == 0x00)&&(pAddr[i+2] == 0x00)));
            ubMP4_V1SpsSize[ubCh] = j;
        }  

        if((pAddr[i] == 0x01)&&(pAddr[i+1] == 0x68))        //PPS
        {
            do
            {    
                ubMP4_V1_Pps[ubCh][j] = pAddr[i+1];
                j++;
                i++;
            }while(!((pAddr[i+1] == 0x00)&&(pAddr[i+2] == 0x00)));
            ubMP4_V1PpsSize[ubCh] = j;
        }         
	}
}
...
void MP4_Init(uint32_t ulStartAdr)
{
    ...
    for(i=0;i<MP4_SRC_NUM;i++)
        MP4_AStszAtomSim[i] = (uint32_t)ulMP4_AllocateMemBuf(MP4_SIMSTSZ_SIZE);

	ubSourceType = ubMP4_SourceType[0];
	ulMP4_StartAddr[0] = (uint32_t)ulMP4_AllocateMemBuf(MP4_STBL_1MINSZ(ubMP4_MaxVFPS,ubMP4_MaxAFPS)*ulMP4_RecTime + MP4_STBL_HDSIZE + MP4_MOOV_HDSIZE);
	
	printd(DBG_InfoLvl, "== MP4_Init ==\r\n");
	for( i = 0 ; i < ubMP4_CfgSrcNum ; i++)
	{
		ubSourceType = ubMP4_SourceType[i];
		
		ulMP4_StartAddr[i] = ulMP4_StartAddr[0];
		printd(DBG_InfoLvl, "Ch:%d Adr: 0x%x \r\n",i, ulMP4_StartAddr[i]);
		ulMP4_FreeAtom[i] = (uint32_t)ulMP4_AllocateMemBuf(MP4_HEAD_SIZE);
		if( ubSourceType == MEDIA_SRC_1V0A || ubSourceType == MEDIA_SRC_1V1A || ubSourceType == MEDIA_SRC_2V1A )
	 	{
	 		// video1
	 	...
	 	if( ubSourceType == MEDIA_SRC_2V1A )
		{
			...
			printd(DBG_InfoLvl, "V2(ts:0x%x ss:0x%x sc:0x%x sz:0x%x co:0x%x)\n",MP4_V2SttsAtom[i], MP4_V2StssAtom[i],MP4_V2StscAtom[i],MP4_V2StszAtom[i],MP4_V2StcoAtom[i]);
	 	}
		printd(DBG_InfoLvl, "\n");
	 	memset(&sRec_MP4Controller[i], 0, sizeof(REC_CHUNK_CONTROLLER));

        ulMP4ThumbnailAdr[i] = (uint32_t)ulMP4_AllocateMemBuf(MP4_THUMBNAIL_SIZE);
        printd(DBG_InfoLvl, "MP4ThumAdr:Start=0x%x, End=0x%x \n", ulMP4ThumbnailAdr[i], ulMP4ThumbnailAdr[i] + MP4_THUMBNAIL_SIZE);
	}
	vMP4_SetLong(MP4_FreeAtomInf, 0, MP4_FREE_ATOM_SIZE);
}
...
{
	return ( (MP4_HEAD_SIZE*ubMP4_CfgSrcNum) + MP4_STBL_HDSIZE*(ubMP4_CfgSrcNum + 1) + MP4_MOOV_HDSIZE + (MP4_THUMBNAIL_SIZE*ubMP4_CfgSrcNum));
}
...
void MP4_CreateFile(uint8_t ubCh)
{
	...
	ulMP4_FileSize[ubCh] = MP4_HEAD_SIZE;
	ulMP4_AtomSize[ubCh] = 0;

	memcpy((uint8_t *)ulMP4_FreeAtom[ubCh],MP4_FtypAtom,sizeof(MP4_FtypAtom));
	ulSize = sizeof(MP4_FtypAtom);
	memcpy((uint8_t *)(ulMP4_FreeAtom[ubCh]+ulSize),MP4_FreeAtomInf,sizeof(MP4_FreeAtomInf));
	ulSize += MP4_FREE_ATOM_SIZE;
	memcpy((uint8_t *)(ulMP4_FreeAtom[ubCh] + ulSize), MP4_MdatAtom, sizeof(MP4_MdatAtom));

    if((ulMP4_V1HSize[ubCh] > 1900 && ulMP4_V1HSize[ubCh] < 1940) && (ulMP4_V1VSize[ubCh] > 1060 && ulMP4_V1VSize[ubCh] < 1100))    // FHD
    {
        ubMP4_V1SpsSize[ubCh] = sizeof (ubMP4_FHD_SPS);
        memcpy(&(ubMP4_V1_Sps[ubCh][0]), ubMP4_FHD_SPS, ubMP4_V1SpsSize[ubCh]);                
    }
    else if((ulMP4_V1HSize[ubCh] > 1260 && ulMP4_V1HSize[ubCh] < 1300) && (ulMP4_V1VSize[ubCh] > 1020 && ulMP4_V1VSize[ubCh] < 1060)) // SXGA
    {
        ubMP4_V1SpsSize[ubCh] = sizeof (ubMP4_SXGA_SPS);
        memcpy(&(ubMP4_V1_Sps[ubCh][0]), ubMP4_SXGA_SPS, ubMP4_V1SpsSize[ubCh]);        
    }
    else if((ulMP4_V1HSize[ubCh] > 1260 && ulMP4_V1HSize[ubCh] < 1300) && (ulMP4_V1VSize[ubCh] > 700 && ulMP4_V1VSize[ubCh] < 740)) // HD
    {
        ubMP4_V1SpsSize[ubCh] = sizeof (ubMP4_HD_SPS);
        memcpy(&(ubMP4_V1_Sps[ubCh][0]), ubMP4_HD_SPS, ubMP4_V1SpsSize[ubCh]);        
    }
    else if((ulMP4_V1HSize[ubCh] > 1004 && ulMP4_V1HSize[ubCh] < 1044) && (ulMP4_V1VSize[ubCh] > 748 && ulMP4_V1VSize[ubCh] < 788)) // XGA
    {
        ubMP4_V1SpsSize[ubCh] = sizeof (ubMP4_XGA_SPS);
        memcpy(&(ubMP4_V1_Sps[ubCh][0]), ubMP4_XGA_SPS, ubMP4_V1SpsSize[ubCh]);        
    }
    else if((ulMP4_V1HSize[ubCh] > 1004 && ulMP4_V1HSize[ubCh] < 1044) && (ulMP4_V1VSize[ubCh] > 580 && ulMP4_V1VSize[ubCh] < 620)) // WSVGA
    {
        ubMP4_V1SpsSize[ubCh] = sizeof (ubMP4_WSVGA_SPS);
        memcpy(&(ubMP4_V1_Sps[ubCh][0]), ubMP4_WSVGA_SPS, ubMP4_V1SpsSize[ubCh]);        
    }
    else if((ulMP4_V1HSize[ubCh] > 780 && ulMP4_V1HSize[ubCh] < 820) && (ulMP4_V1VSize[ubCh] > 580 && ulMP4_V1VSize[ubCh] < 620)) // SVGA
    {
        ubMP4_V1SpsSize[ubCh] = sizeof (ubMP4_SVGA_SPS);
        memcpy(&(ubMP4_V1_Sps[ubCh][0]), ubMP4_SVGA_SPS, ubMP4_V1SpsSize[ubCh]);        
    }
    else if((ulMP4_V1HSize[ubCh] > 780 && ulMP4_V1HSize[ubCh] < 820) && (ulMP4_V1VSize[ubCh] > 460 && ulMP4_V1VSize[ubCh] < 500)) // WVGA
    {
        ubMP4_V1SpsSize[ubCh] = sizeof (ubMP4_WVGA_SPS);
        memcpy(&(ubMP4_V1_Sps[ubCh][0]), ubMP4_WVGA_SPS, ubMP4_V1SpsSize[ubCh]);        
    }
    else if((ulMP4_V1HSize[ubCh] > 620 && ulMP4_V1HSize[ubCh] < 660) && (ulMP4_V1VSize[ubCh] > 460 && ulMP4_V1VSize[ubCh] < 500))   // VGA
    { 
        ubMP4_V1SpsSize[ubCh] = sizeof (ubMP4_VGA_SPS);
        memcpy(&(ubMP4_V1_Sps[ubCh][0]), ubMP4_VGA_SPS, ubMP4_V1SpsSize[ubCh]);
    }
    ubMP4_V1PpsSize[ubCh] = sizeof (ubMP4_PPS);
    memcpy(&(ubMP4_V1_Pps[ubCh][0]), ubMP4_PPS, ubMP4_V1PpsSize[ubCh]);
	
	FS_WriteFile(MP4_FS_MediaSel, (FS_SRC_NUM)ubCh, (uint32_t)(ulMP4_FreeAtom[ubCh]) , ulMP4_FileSize[ubCh], NULL);
}

int32_t slMP4_WriteOneFrame(uint8_t ubCh, uint8_t ubStreamType,uint8_t ubFrameType, uint32_t ulAddr, uint32_t ulSize, uint32_t ulDesID,uint8_t ubFrmSeq)
{
	//printd(MP4_LOGINFO,"\r\nWF ST:%d FT:%d Sz:%d\r\n",ubStreamType,ubFrameType,ulSize);
	if(osSemaphoreWait(SEM_MP4_WRFrm ,3) != osOK)
	{
		printd(DBG_CriticalLvl,"SEM_MP4_WRFrm Busy\r\n");
		return -10;
	}
	...
	if(ubStreamType == MP4_STR_A1)
	{
			...
			( (sRec_MP4Controller[ubCh].ubChunkStartFlag == 1) && (sRec_MP4Controller[ubCh].ubVideoAudioRunFg[ubStreamType] == 0)))
		{
...
#elif (MP4_FORMAT_MODE == MP4_FORMAT_ALAW)
			sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulSttsSampleCount += ulSize;
			sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulSttsSampleDuration = 1;
#endif
...
#elif (MP4_FORMAT_MODE == MP4_FORMAT_ALAW)
			sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStszSampleSizeEntry += ulSize;				//stsz
#endif	
			...
			sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStcoChunkOffset = ulMP4_FileSize[ubCh];
			sRec_MP4Controller[ubCh].ubStreamType = ubStreamType;
			//printd(MP4_LOGINFO,"ST:%d LEN:0x%x\r\n",ubStreamType,ulMP4_FileSize[ubCh]);
		}
		else
		{
#if (MP4_FORMAT_MODE == MP4_FORMAT_AAC)		
...
#elif (MP4_FORMAT_MODE == MP4_FORMAT_ALAW)
			vMP4_AdoCheckUpdateStts(ubCh,(uint8_t *)MP4_ASttsAtom[ubCh],ubStreamType,ulSize,1);	    // stts
			vMP4_AdoCheckUpdateStsc(ubCh,(uint8_t *)MP4_AStscAtom[ubCh],(uint8_t *)MP4_AStcoAtom[ubCh],ubStreamType,ulSize,ulDesID);    // stsc,stco
			sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStszSampleSizeEntry += ulSize;
#endif				
		...
		FS_WriteFile(MP4_FS_MediaSel, (FS_SRC_NUM)ubCh, (uint32_t)ulAddr, sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStszSampleSize, NULL);
	...
	{
		//printd(MP4_LOGINFO,"Z:%d %d %d\r\n",ubFrameType ,sRec_MP4Controller[ubCh].ubChunkStartFlag, sRec_MP4Controller[ubCh].ubVideoAudioRunFg[ubStreamType]);
#if (MP4_FORMAT_MODE == MP4_FORMAT_AAC)
		vMP4_CheckSourceSWUpdateChunk(ubCh,ubStreamType);
#elif (MP4_FORMAT_MODE == MP4_FORMAT_ALAW)
		if(sRec_MP4Controller[ubCh].ubStreamType == MP4_STR_A1)
		    vMP4_AdoChkSrcSWUpdateChunk(ubCh,ubStreamType,ulSize);
		else
		    vMP4_CheckSourceSWUpdateChunk(ubCh,ubStreamType);
#endif	
		...
		if(ubFrameType == MP4_V_IFRM)	// I frame
		{
            ...
            MP4_GetSpsPpsSize(ubCh, (uint8_t *)(ulAddr));
			// mdat

                sRec_MP4Controller[ubCh].ulmdatSampleSize[ubStreamType] = ulSize-4-(ubMP4_V1SpsSize[ubCh] + ubMP4_V1PpsSize[ubCh] + 8);     //(4 + SpsSize + 4 + PpsSize)          
			if(ubStreamType == MP4_STR_V1)
			{
            ...
            #if MP4_transfer_bitstream_to_MP4_format
            memcpy(&ubMP4_StreamHD[ubCh][0], (uint8_t *)ulAddr , 32);
            *(ubMP4_StreamHD[ubCh] + 3) = ubMP4_V1SpsSize[ubCh];	// SPS size //0x67
            *(ubMP4_StreamHD[ubCh]+ 4 + ubMP4_V1SpsSize[ubCh] + 3) = ubMP4_V1PpsSize[ubCh];	// PPS size //0x68
            vMP4_SetLong(&ubMP4_StreamHD[ubCh][0], (4 + ubMP4_V1SpsSize[ubCh] + 4 + ubMP4_V1PpsSize[ubCh]), sRec_MP4Controller[ubCh].ulmdatSampleSize[ubStreamType]); //4: size field, 0xa+0x4+4+4: SPS+PPS+two size fields			#endif			
            sHdMod[ubCh].ulStartOfs = 0;
            sHdMod[ubCh].ulSrcAddr = (uint32_t)&ubMP4_StreamHD[ubCh][0];
            sHdMod[ubCh].ulSize = 32;
            #endif
            
            ubMP4_PIC_TOGGLE[ubCh] = 0;
			ubMP4_PIC[ubCh] = 0 + 2;
			ubMP4_POC[ubCh] = 1 + 2;
			
			//save mdat data
			vREC_FrameEndPointEx(ubCh,REC_I_VFRM,REC_WR_SROTRAGE_BF,ubFrmSeq);
			FS_WriteFile(MP4_FS_MediaSel, (FS_SRC_NUM)ubCh, (uint32_t)ulAddr, sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStszSampleSize,&sHdMod[ubCh]);
            vREC_FrameEndPointEx(ubCh,REC_I_VFRM,REC_WR_SROTRAGE_AF,ubFrmSeq);
			ulMP4_FileSize[ubCh] += sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStszSampleSize;
		}
		else	if((ubFrameType == MP4_V_PFRM) && (sRec_MP4Controller[ubCh].ubChunkStartFlag == 1) && (sRec_MP4Controller[ubCh].ubVideoAudioRunFg[ubStreamType] == 1))	// P frame
		{
            ...
            #if MP4_transfer_bitstream_to_MP4_format
            memcpy(&ubMP4_StreamHD[ubCh][0], (uint8_t *)ulAddr , 32);
            vMP4_SetLong(&ubMP4_StreamHD[ubCh][0], 0, sRec_MP4Controller[ubCh].ulmdatSampleSize[ubStreamType]);
            *(ubMP4_StreamHD[ubCh] + 6) = ubMP4_PIC[ubCh];
            *(ubMP4_StreamHD[ubCh] + 7) = ubMP4_POC[ubCh];
            ...
            if(ubMP4_PIC[ubCh] >= 0xFE) // PIC is Overflow
            {
                if(ubMP4_PIC_TOGGLE[ubCh])  ubMP4_PIC_TOGGLE[ubCh] = 0;
                else    ubMP4_PIC_TOGGLE[ubCh] = 1;
            }
			...
			//save mdat data
			vREC_FrameEndPointEx(ubCh,REC_P_VFRM,REC_WR_SROTRAGE_BF,ubFrmSeq);
			FS_WriteFile(MP4_FS_MediaSel, (FS_SRC_NUM)ubCh, (uint32_t)ulAddr, sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStszSampleSize,&sHdMod[ubCh]);
            vREC_FrameEndPointEx(ubCh,REC_P_VFRM,REC_WR_SROTRAGE_AF,ubFrmSeq);
			ulMP4_FileSize[ubCh] += sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStszSampleSize;
		}
		else if((ubFrameType == MP4_V_SFRM) && (sRec_MP4Controller[ubCh].ubChunkStartFlag == 1))		//skip frame
		{
            ...
            if(ubMP4_PIC_TOGGLE[ubCh])
            {
                *(ubMP4_StreamHD[ubCh] + 5) = 0x9B;
            }
            ...
            *(ubMP4_StreamHD[ubCh] + 7) = ubMP4_POC[ubCh];
            ...
            sHdMod[ubCh].ulSrcAddr = (uint32_t)&ubMP4_StreamHD[ubCh][0];
            sHdMod[ubCh].ulSize = 32;
			*(uint8_t *)(ulAddr+6) = ubMP4_PIC[ubCh];       //S
...
void MP4_Close(uint8_t ubCh, FS_FILE_SUB_HIDN_INFO_t HidnInfo)
{
	int32_t slLength = 0;
	uint32_t i = 0;
	uint32_t ulTemp = 0;
    ...
    //update   MP4_V1StsdAtom MP4_V1Avc1Atom
    MP4_V1Avc1Atom[MP4_AVCC_SIZE] =  14 + 2 + ubMP4_V1SpsSize[ubCh] + 3 + ubMP4_V1PpsSize[ubCh];    //
    MP4_V1Avc1Atom[3] = 0x56 + MP4_V1Avc1Atom[MP4_AVCC_SIZE];                   //MP4_V1Avc1Atom Size
    
    MP4_V1StsdAtom[3] =  MP4_V1Avc1Atom[3] + sizeof(MP4_V1StsdAtom);          //V1Stsd
    MP4_V1Avc1Atom[MP4_AVC1_SPS_SIZE] = ubMP4_V1SpsSize[ubCh];
    memcpy(&MP4_V1Avc1Atom[MP4_AVC1_SPS],ubMP4_V1_Sps[ubCh],ubMP4_V1SpsSize[ubCh]);
    
    MP4_V1Avc1Atom[MP4_AVC1_SPS + ubMP4_V1SpsSize[ubCh]] = 0x01;
    MP4_V1Avc1Atom[MP4_AVC1_SPS + ubMP4_V1SpsSize[ubCh] + 1] = 0x00;
    MP4_V1Avc1Atom[MP4_AVC1_SPS + ubMP4_V1SpsSize[ubCh] + 2] = ubMP4_V1PpsSize[ubCh];  
    memcpy(&MP4_V1Avc1Atom[MP4_AVC1_SPS + ubMP4_V1SpsSize[ubCh] + 3],ubMP4_V1_Pps[ubCh],ubMP4_V1PpsSize[ubCh]);
    
	// Update mdata Size
	ulmdatSz = ulMP4_FileSize[ubCh] - (MP4_FTYPE_ATOM_SIZE+MP4_FREE_ATOM_SIZE);
	vMP4_SetLong(MP4_MdatAtom, 0, ulmdatSz);

	// Update stream info.
	vMP4_UpdateVideoAtom(ubCh,MP4_STR_V1);

	if(	ubMP4_SourceType[ubCh] == MEDIA_SRC_1V0A || ubMP4_SourceType[ubCh] == MEDIA_SRC_2V1A || ubMP4_SourceType[ubCh] == MEDIA_SRC_1V1A)
    ...
    vMP4_SaveAtom(ubCh,MP4_V1Avc1Atom, MP4_V1Avc1Atom[3]);
	...
	if(	ubMP4_SourceType[ubCh] == MEDIA_SRC_1V0A || ubMP4_SourceType[ubCh] == MEDIA_SRC_1V1A || ubMP4_SourceType[ubCh] == MEDIA_SRC_2V1A)
	{
		...
		MP4_Mp4aAtom[32] = (ulMP4_AdoSampleRate[ubCh] & 0x0000FF00) >>8;
		MP4_Mp4aAtom[33] = ulMP4_AdoSampleRate[ubCh] & 0x000000FF;
	...
	for(i = 0; i < ulQuot ; i++)
	{
		while( FS_WriteFile(MP4_FS_MediaSel, (FS_SRC_NUM)ubCh, (uint32_t)(ulMP4_StartAddr[ubCh] +  (0x20000 * i)), 0x20000, NULL) != FS_SEND_Q_SUCCESS )
		{
            if(FS_ChkMediaExist(MP4_FS_MediaSel) == FS_MEDIA_OUT)
                return;
            if(FS_ChkMediaRdy(MP4_FS_MediaSel) == FS_MEDIA_NOT_RDY)
                return;
			printd(DBG_CriticalLvl,"1.write fail !! write again\n");
			osDelay(50);
		}// 128K
	}
	...
	if(ulRem > 0)
	{
		while( FS_WriteFile(MP4_FS_MediaSel, (FS_SRC_NUM)ubCh, (uint32_t)(ulMP4_StartAddr[ubCh] +  (0x20000 * i)), ulRem, NULL) != FS_SEND_Q_SUCCESS )
		{
            if(FS_ChkMediaExist(MP4_FS_MediaSel) == FS_MEDIA_OUT)
                return;
            if(FS_ChkMediaRdy(MP4_FS_MediaSel) == FS_MEDIA_NOT_RDY)
                return;
			printd(DBG_CriticalLvl,"2.write fail !! write again\n");
			osDelay(50);
		}// 128K
	}
	...
	memcpy((uint8_t *)ulMP4ThumbnailAdr[ubCh], MP4_UdtaAtom, sizeof(MP4_UdtaAtom));
    vMP4_SetLong((uint8_t *)(ulMP4ThumbnailAdr[ubCh]+8), 0, ulMP4ThumbnailSize[ubCh]);
	FS_WriteFile(MP4_FS_MediaSel, (FS_SRC_NUM)ubCh, (uint32_t)ulMP4ThumbnailAdr[ubCh], MP4_THUMBNAIL_SIZE, NULL);

	// update mdate size.
	memcpy((uint8_t *)(ulMP4_FreeAtom[ubCh]),MP4_FtypAtom,sizeof(MP4_FtypAtom));
	memcpy((uint8_t *)(ulMP4_FreeAtom[ubCh] + MP4_FTYPE_ATOM_SIZE),MP4_FreeAtomInf,sizeof(MP4_FreeAtomInf));
	memcpy((uint8_t *)(ulMP4_FreeAtom[ubCh] + (MP4_FTYPE_ATOM_SIZE+MP4_FREE_ATOM_SIZE)), MP4_MdatAtom, sizeof(MP4_MdatAtom));
	FS_UpdateMp4Header(MP4_FS_MediaSel, (FS_SRC_NUM)ubCh,(uint32_t)ulMP4_FreeAtom[ubCh], MP4_HEAD_SIZE);
	FS_CloseFile(MP4_FS_MediaSel, (FS_SRC_NUM)ubCh, &HidnInfo);
	
	printd(MP4_LOGINFO,"\r\n");
}

uint32_t ulMP4_AlignMoveLength(uint8_t ubCh,uint8_t ubStreamType)
{
    uint32_t ulTemp;
    uint16_t uwFrmInt;
    uint32_t ulRtn=0;
    ...
    if(ubStreamType == MP4_STR_V1) 
    {
        ulRtn = sRec_MP4Controller[ubCh].stStbl[ubStreamType].ulStszSampleSizeEntry;
        return ulRtn;
    }
    else if(ubStreamType == MP4_STR_A1)
    {
        uwFrmInt = uwMP4_FrmDuration[ubCh][MP4_STR_A1]*1000/ulMdhdTimeScale[ubCh][MP4_STR_A1];
        if(ulMdhdDuration[ubCh][MP4_STR_V1] > ((ulMdhdDuration[ubCh][MP4_STR_A1]*(ulMvhdTimeScale[ubCh]/10))/(ulMdhdTimeScale[ubCh][MP4_STR_A1]/10)))
        {
            ulTemp = ulMdhdDuration[ubCh][MP4_STR_V1] - ((ulMdhdDuration[ubCh][MP4_STR_A1]*(ulMvhdTimeScale[ubCh]/10))/(ulMdhdTimeScale[ubCh][MP4_STR_A1]/10));
            ulRtn = ulTemp / uwFrmInt;
            if(ulTemp % uwFrmInt)
                ulRtn++;
        }
        return ulRtn;
...
void MP4_ClearUserDataSize(uint8_t ubCh)
{
    memset((uint8_t *)ulMP4ThumbnailAdr[ubCh],0x00,MP4_THUMBNAIL_SIZE);
    ulMP4ThumbnailSize[ubCh] = 4;
}

void MP4_WriteUserData(uint8_t ubCh, uint8_t *ptr, uint32_t ulSize)
{
    memcpy((uint8_t *)(ulMP4ThumbnailAdr[ubCh] + 8 + ulMP4ThumbnailSize[ubCh]), ptr, ulSize);
    ulMP4ThumbnailSize[ubCh] += ulSize;
}
...
void vMP4_GetHeadInfo(PLY_MP4HD_INFO *PlyMP4HdInfo)
{
    PlyMP4HdInfo->uwFTypeSz = MP4_FTYPE_ATOM_SIZE;
    PlyMP4HdInfo->ulFreeSz = MP4_FREE_ATOM_SIZE;
    PlyMP4HdInfo->uwMdatSz = MP4_MDAT_ATOM_SIZE;
    PlyMP4HdInfo->ulHdTotalSz = MP4_HEAD_SIZE;
}
...
void MP4_ParsingAudioFormat(uint8_t ubCh, uint32_t ulAdr)
{
    if(memcmp((uint8_t*)(ulAdr+28), "alaw", 4)==0)
        ubMP4_FormatPly[ubCh] = MP4_FORMAT_ALAW;
    else if(memcmp((uint8_t*)(ulAdr+28), "mp41", 4)==0)
        ubMP4_FormatPly[ubCh] = MP4_FORMAT_AAC;
    else
        ubMP4_FormatPly[ubCh] = MP4_FORMAT_ALAW;
}

uint8_t MP4_AssignmentStructure(uint8_t ubCh, uint32_t ulAdr, uint32_t ulSize)
{
	uint32_t ulAtomSz = 0;
	uint32_t ulOffset = 0;
    ...
    if( memcmp(&pMoov[ubCh]->ubAtomType[0], "moov", 4)!=0 )
    {
        printf("MP4 Header moov Info fail\n");
        return 0;
    }
    ...
    ulAdr += (ulMP4_GetLong((uint8_t*)(pStsdV[ubCh]),0) -sizeof(VIDEO_SAMPLE_DESCRIPTION_ATOM));                  // MP4_V1StsdAtom def size 0x89     
...
uint32_t ulMP4_GetTotalFrameNumber(uint8_t ubCh, uint8_t ubStreamType)
{
    ...
    {
        if(ubMP4_FormatPly[ubCh] == MP4_FORMAT_AAC)
            return ulMP4_GetLong((uint8_t*)(sStbl_Info[ubCh].ulSttsAAdr),16);
        else if(ubMP4_FormatPly[ubCh] == MP4_FORMAT_ALAW)
            return ulMP4_GetLong((uint8_t*)(sStbl_Info[ubCh].ulStcoAAdr),12);
    }
	return 0;
}
...
uint8_t ubMP4_GetResolution(uint8_t ubCh, uint8_t ubStreamType) 
{
	...
	if( (1900 < ulWidth && 1940 > ulWidth) && (1060 < ulHeight && 1100 > ulHeight))
		...
		return MP4_RES_WVGA;
	else if ( (620 < ulWidth && 660 > ulWidth) && (450 < ulHeight && 500 > ulHeight))
		return MP4_RES_VGA;
	else
		return MP4_RES_NONE;
}
...
uint32_t ulMP4_GetFrameDuration(uint8_t ubCh, uint8_t ubStreamType)
{
	...
	else if( ubStreamType == MP4_STR_A1)
	{
        ...
        {
            uint32_t ulSampleCnt,ulSampleDuration,ulTotFrm;
    	    ulSampleCnt = ulMP4_GetLong((uint8_t*)(sStbl_Info[ubCh].ulSttsAAdr),16);
    	    ulSampleDuration = ulMP4_GetLong((uint8_t*)(sStbl_Info[ubCh].ulSttsAAdr),20);
    	    ulTotFrm = ulMP4_GetLong((uint8_t*)(sStbl_Info[ubCh].ulStcoAAdr),12);
    	    return ((ulSampleCnt*ulSampleDuration)/ulTotFrm);
        }
	}
	return 0;
}
...
uint32_t ulMP4_Parse1stIFrameInfo(uint8_t ubCh, uint32_t ulAdr,PLY_CHUNK_CONTROLLER *PlyChkCtrl)
{
	uint32_t ulStcoEntryN = 0;	// total chunk
	uint32_t ulStcoIdx = 0;
	uint32_t ulStcoAdr = 0;
	...
	for( ; ulChunkCnt <= ulStcoEntryN ; ulChunkCnt++)
	{
		...
		for(ulFrmInChkCnt = 1 ; ulFrmInChkCnt <= ulStscFrmInChk ; ulFrmInChkCnt++)
		{
			...
			if(ulFrmCnt == ulStssFrm)
			{
			    ulStszSz = ulMP4_GetLong((uint8_t*)(sStbl_Info[ubCh].ulStszVAdr),(20 + (4*ulStszIdx)));
				memcpy((uint8_t *)(ulAdr + 8 * ulStssIdx), (uint8_t *)&ulStcoAdr, 4);
				memcpy((uint8_t *)(ulAdr + 8 * ulStssIdx + 4), (uint8_t *)&ulStszSz, 4);
				ulStssIdx++;
				PlyChkCtrl->ulVDOTotalSTSC = ulStscEntryN;
				PlyChkCtrl->ulVDOTotalChk =  ulStcoEntryN;
				PlyChkCtrl->ulVDOChkIdx = 0;	
...
uint32_t ulMP4_ParseAudioFrameTable(uint8_t ubCh, uint32_t ulAdr,PLY_CHUNK_CONTROLLER *PlyChkCtrl)
{
    ...
    else if(ubMP4_FormatPly[ubCh] == MP4_FORMAT_ALAW)
    {
    	...
    	uint32_t ulStscIdx = 0;
    	uint32_t ulStscChkID = 0;
    	uint32_t ulStscFrmSz = 0;

    	ulStcoEntryN = ulMP4_GetLong((uint8_t*)(sStbl_Info[ubCh].ulStcoAAdr),12);
    	ulStscEntryN = ulMP4_GetLong((uint8_t*)(sStbl_Info[ubCh].ulStscAAdr),12);

    	memset((uint8_t *)MP4_AStszAtomSim[ubCh],0x00,MP4_SIMSTSZ_SIZE);
    	vMP4_SetLong((uint8_t *)(MP4_AStszAtomSim[ubCh]),16,ulStcoEntryN);
    	for( ulChunkCnt = 1 ; ulChunkCnt <= ulStcoEntryN ; ulChunkCnt++)
    	{
    	    if( ulStscIdx < ulStscEntryN)
    	    {
                ulStscChkID = ulMP4_GetLong((uint8_t*)(sStbl_Info[ubCh].ulStscAAdr),(16 + (12*ulStscIdx)));
                if(ulChunkCnt == ulStscChkID)
                {
        			ulStscFrmSz = ulMP4_GetLong((uint8_t*)(sStbl_Info[ubCh].ulStscAAdr),(20 + (12*ulStscIdx)));
        			ulStscIdx++;
        		}
    		}
    		vMP4_SetLong((uint8_t *)(MP4_AStszAtomSim[ubCh]),(16+4*ulChunkCnt),ulStscFrmSz);
    	}

    	for( ulChunkCnt = 1 ; ulChunkCnt <= ulStcoEntryN ; ulChunkCnt++)
    	{
    		ulStcoAdr = ulMP4_GetLong((uint8_t*)(sStbl_Info[ubCh].ulStcoAAdr),(16 + (4*ulStcoIdx)));
    		memcpy((uint8_t *)(ulAdr + 8*ulStcoIdx), (uint8_t *)&ulStcoAdr, 4);
    		memcpy((uint8_t *)(ulAdr + 8*ulStcoIdx + 4), (uint8_t *)&ulChunkCnt, 4);
    		ulStcoIdx++;
    	}
        PlyChkCtrl->ulADOTotalSTSC = ulStscEntryN;
    	PlyChkCtrl->ulADOTotalChk =  ulStcoEntryN;
    	...
    	PlyChkCtrl->ulADOTotalFrm = ulStcoEntryN;
    	PlyChkCtrl->ulADOFrmIdxInFile = 0;
    	PlyChkCtrl->ulADOFrmSize = 0;
    }
    return 1;
...
uint8_t ubMP4_GetAudioCodec(uint8_t ubCh)
{
    if(pStsdA[ubCh]->ubDescriptionSize[4] == 0x6D &&      // aac
       pStsdA[ubCh]->ubDescriptionSize[5] == 0x70 &&
       pStsdA[ubCh]->ubDescriptionSize[6] == 0x34 &&
       pStsdA[ubCh]->ubDescriptionSize[7] == 0x61 )
       return AAC;
    else if(pStsdA[ubCh]->ubDescriptionSize[4] == 0x61 && //alaw
       pStsdA[ubCh]->ubDescriptionSize[5] == 0x6C &&
       pStsdA[ubCh]->ubDescriptionSize[6] == 0x61 &&
       pStsdA[ubCh]->ubDescriptionSize[7] == 0x77 )
       return ALAW;
    else
	    return ALAW;
}
...
uint32_t ulMP4_ParseMdatFrameInfo(uint8_t ubCh,PLY_CHUNK_CONTROLLER *PlyChkCtrl)
{
	...
	if( PlyChkCtrl->ulFrmType == MP4_FRMTYPE_VDO)
	{
        ...
        if(PlyChkCtrl->ulVDOFrmSize > 0x300000)
            printf("MP4 %d %X\n",ubCh,PlyChkCtrl->ulVDOFrmSize);
	}
	else if( PlyChkCtrl->ulFrmType == MP4_FRMTYPE_ADO)
	{
        if(ubMP4_FormatPly[ubCh] == MP4_FORMAT_AAC)
			...
			PlyChkCtrl->ulADOFrmSize = ulMP4_GetLong((uint8_t*)(MP4_AStszAtomSim[ubCh]),(20+ (4*PlyChkCtrl->ulADOFrmIdxInFile)));
...
uint32_t ulMP4_JumpReload(uint8_t ubCh, PLY_CHUNK_CONTROLLER *PlyChkCtrl)
{
	...
	if(PlyChkCtrl->ulFrmType == MP4_FRMTYPE_ADO)
	{
		...
		if( PlyChkCtrl->ulFileOffset == MP4_HEAD_SIZE)
		{
			PlyChkCtrl->ulVDOChkIdx = 0;
			PlyChkCtrl->ulVDOChkAdrOffset = ulMP4_GetLong((uint8_t*)(sStbl_Info[ubCh].ulStcoVAdr),(16 + (4*PlyChkCtrl->ulVDOChkIdx)));
			PlyChkCtrl->ulVDOFrmIdxInFile = 0;
	...
	else if(PlyChkCtrl->ulFrmType == MP4_FRMTYPE_VDO)
	{
		...
		else	
		{
            ...
            if(ubMP4_FormatPly[ubCh] == MP4_FORMAT_AAC)
            {
                ...
                PlyChkCtrl->ulADOFrmIdxInFile = PlyChkCtrl->ulADOChkIdx;
		}
	}
